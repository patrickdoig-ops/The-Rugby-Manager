...
**WP 6 status (🟡 authoring half landed; overlay engine + UI outstanding):** the parts a manager *authors* are live on main; the part that *fires plays in a live match* is the remaining build.
...
- *Selection + defensive familiarity* — **🟡 PhasePlay landed; FirstPhase strike selection next.** `src/engine/playSelection.ts`: `selectPlay()` rolls a fire gate (`PLAY_SELECTION.firePct = 9`) on the OUTCOME stream, filters `PLAYBOOK` by trigger, and picks weighted-random by `styleAffinity` (the team's effective `attackingStyle` IS the "default playbook" — no stored per-team list) × `(1 − familiarityWeightDrop × recency)`. `handlePhasePlay` calls it on every spatial carry: a hit emits `PLAY_SELECTED` (bumps `state.playRecency[side][playId]`, decays the side's others — a new `MatchEvent` variant through `applyMatchEvent`; the match is transient so no save bump) and overlays the carry. **Familiarity bites BOTH** (owner call): recency lowers the selection weight (variety) AND widens the overlay's abort radii (`PLAY_OVERLAY.familiarityReadGain = 0.4` — defenders read a repeated move). Tuned **neutral** to the owner's post-rebalance § 13 targets (5-seed tries 5.67, pts 36.99 — both in band); only the silent-score golden regenerated (`211b17b0…`). Two new scenarios guard it (familiarity read + decline; 33 total). **Outstanding:** strike-play selection wired into `FirstPhaseEvent` (off scrum/lineout). Constants in `balance/spatialDecision.ts` (`PLAY_SELECTION`).
