...
npm run probe     # headless-Chromium capture of the 2D pitch animation (scripts/pitchProbeDriver.mjs + pitch-probe.html + scripts/pitchProbe.ts). Mounts the REAL PitchView against a REAL match, screenshots set-piece / first-phase / kick-off beats mid-animation, and dumps a frame-by-frame ball/dot trace → `harness/` (gitignored). Reuses a running `npm run dev`, else spawns its own Vite. Use to sense-check ball/dot motion you can't see from static code. Browser = @sparticuz/chromium + puppeteer-core (registry-hosted; the Playwright CDN is blocked in cloud sandboxes).
```
