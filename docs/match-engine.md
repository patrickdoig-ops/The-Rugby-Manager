...
| `src/engine/spatial/ShapeSolver.ts` (WP 2; continuous shape WP 5) | Layer-1 ROLE solver: `solveDefence` (line slots anchored at the mark, fold via `speedScale`, backfield, returns line roles), `reanchorDefence` (per-tick line re-anchor — see "Per-tick line re-anchor"), `solveCarryCorridor` (the carrier **engages from the ruck**: snapped onto the mark each beat then targets `mark + attackDir·carryReach` forward — so a continuation-beat carrier from out in the shape comes to the breakdown instead of dragging the ball back; plus the support pod, all `role: 'corridor'`), `solveAttackSpread` (opening positions for the off-ball attackers — forwards in a loose cluster behind the mark, backs fanning to the open side with width + depth), **`reanchorSupport`** (per-tick: trails the support pod `supportDepth` behind the carrier so cleaners stay ONSIDE), **`reanchorAttack`** (per-tick: holds the off-ball forward cluster + backline at fixed depth BEHIND the live gain line = the carrier's x, keeping width — the whole attack stays onside as the carry advances), `detectGap` (lateral-dominant gap → line-break verdict + nearest line defender), `detectOffside` (creep → offside offender). Assigns destinations + `speedScale` only — never moves anyone (position seeding is `seedFormation`; per-tick movement is `MovementSystem`), except `solveCarryCorridor`'s one-player carrier-to-mark snap. |
