...
Time advances `0.2 + rng(0,8)/10` game minutes per tick (0.2–1.0 min). Fatigue is applied every ~5 accumulated game minutes via `fatigueAccumulator`. Clock is clamped to 40 (first half) or 80 (second half) until `clockInTheRed` is set, then advances at 1/20 normal speed.
...
Clock reaches 40 min (first half) or 80 min (second half) → clockInTheRed = true, commentary emitted, clock slows to 1/20 speed.
