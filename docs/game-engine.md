...
| `MATCHDAY_ADVANCED` | After a cup / European matchday's result is recorded (the player's match + the rest of that matchday simmed) | Sets `calendar.date = toDate` (the next cup/European/league matchday) and nothing else. `calendar.week` (the league-round cursor) is left untouched, so league scheduling, standings, break detection and `upcomingGap` are unaffected. Consumes no RNG; runs none of `WEEK_ADVANCED`'s league-round-scoped passes. The matchday drains in the post-match `afterTraining` tail before the Hub re-surfaces the next league-round CTA. |
