...
| `loanPoolGenerator.ts` | `buildLoanPoolEvents(state)` — called by `GameCoordinator.newSeason` after staff seeding, then at the end of every `rollSeason()` (the pool is replaced each season; the previous pool's players persist in the roster as orphaned records). Generates 15–20 loan-pool players via `generatePersona` (ages 19-26, OVR 55-72), fires one `FOREIGN_IMPORT_ARRIVED` per player + one `LOAN_POOL_SEEDED` to register them and remove them from `freeAgents`. |
