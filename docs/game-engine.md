...
| `loanPoolGenerator.ts` | `buildLoanPoolEvents(state)` — called by `GameCoordinator.newSeason` after staff seeding. Generates 15–20 loan-pool players via `generatePersona` (ages 19-26, OVR 55-72), fires one `FOREIGN_IMPORT_ARRIVED` per player + one `LOAN_POOL_SEEDED` to register them and remove them from `freeAgents`. |
...
| `SUMMER_TOUR_RETURN_SET` | Per matched England / Wales summer-tour player at the 2025/26 `newSeason` | Sets `Player.summerTourReturn = true` + the reduced return `condition`. No stand-down; players are immediately available (English players excluded from pre-season cup leg-0 only, computed on-the-fly in `runPreSeasonBlock`). |
...
| `PLAYING_TIME_PROMISED` | `GameCoordinator.makePlayingTimePromise(rosterId)` — inbox "Promise game time" CTA | Sets `Player.playingTimePromise = { toRound: week + 4, startsRequired: 3, startsAtPromise: seasonStats.starts }`; clears `wantsTransfer`. Checked each `WEEK_ADVANCED` — if `toRound` is passed and the player hasn't accumulated `startsRequired` starts since the promise, fires `PROMISE_BROKEN`. |
...
| `TRANSFER_REQUEST_REJECTED` | `GameCoordinator.rejectTransferRequest(rosterId)` — inbox "Reject" CTA | Clears `Player.wantsTransfer`; fires `PLAYER_MORALE_ADJUSTED` with `delta = MORALE.transferRequestRejectPenalty (−8)`. |
| `PROMISE_BROKEN` | `TransferCoordinator.checkTransferRequestsAndPromises()` when `toRound` has passed with insufficient starts | Clears `Player.playingTimePromise`; fires `PLAYER_MORALE_ADJUSTED` with `delta = MORALE.promiseBrokenPenalty (−15)`. No forced sale. |

**Loan system (Feature 2.3):**

| Variant | When fired | What it does |
|---|---|---|
| `LOAN_POOL_SEEDED` | `GameCoordinator.newSeason` (after staff seeding) and `fromSave` (when `save.career.loanPool` is present) | Sets `state.career.loanPool` to the array of rosterIds; removes those ids from `state.career.freeAgents` so pool players don't appear in the transfer market. |
...
| `medical` | Any squad player with `summerTourReturn === true` at week 1 — lists England/Wales summer-tour returners and their return condition (immediately available, no stand-down) | `squad` |
