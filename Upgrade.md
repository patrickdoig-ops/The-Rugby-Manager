...
- *Selection + defensive familiarity* — **⬜ outstanding.** Play choice inside the WP5 carrier/playmaker utility scoring + a per-match recency familiarity penalty; AI sides get a default playbook from `suggestedTactics`. Constants in `balance/spatialDecision.ts`.
