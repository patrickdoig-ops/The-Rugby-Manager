...
| **6. Playbook 🟡** | [WP6](docs/upgrade/WP6-playbook.md) | overlay Opus / UI Sonnet | Play-overlay system + schema, initial ~10-move library (§ 7.1), **play editor** (§ 9), familiarity penalty, match-day playbook UI (§ 7.2) | nothing (additive) | strike-play scenarios (miss-2 vs fold speeds); play-abort rate credible |
...
- *Three-layer stack formalisation* (§ 6) — **🟡 legibility done; new behaviours deferred.** The three layers are now explicitly TAGGED at their seams and surfaced in the frame-debugger annotation (the "why is he there?"): **Layer 1 ROLE** (ShapeSolver shape/fold — `setAttackTarget`/`setFoldTarget`), **Layer 2 DECIDE** (ruck commitment vetoing the shape — `steerCommitment`), **Layer 3 REACT** (contact hard interrupt, a beaten defender — `ContactSystem`). Dev-only (`recordAnnotations`), zero outcome impact. The *new* Layer-2/3 behaviours the plan also lists (a winger-holds-width veto, a ball-in-air interrupt) are additive and **deferred** — the existing vetoes/interrupts (RuckCommitment, contact) are the ones that exist and are now annotated.
...
- *Shape editor* (§ 9) — **✅ landed.** The Phase Animator gained a subset roster (author a shape for any subset — one team's backs, all 15, etc. — excluded players are left out of the export) + an "Export spatial shape" button; the spatial engine **consumes** authored shapes: `AUTHORED_ATTACK_SHAPES[attackingStyle]` (`balance/attackShapes.ts`) drives the named slots into a hand-authored formation (mark-relative, attack-oriented, open-side-mirrored), the rest fall back to procedural pods/backline. The seeded `wide_wide` shape spreads the backline ≈ 61 m. (Per-team/per-phase shape *selection* and named set-moves are the **WP6 playbook**.)
- *Try-channel telemetry metric* — needs `TRY_SCORED` to carry the channel (a frozen-event-shape change). (The composure-noise scenario it was paired with is now unblocked — the carrier utility AI it depended on has landed, increment 6 above.)
- *Watch test* — owner sign-off (machines can't self-certify).

**WP 6 status (🟡 engine complete — plays fire in live matches; only the match-day UI outstanding):** the parts a manager *authors* and the part that *fires plays in a live match* (overlay engine + selection + familiarity) are live on main; the remaining build is the manager-facing playbook picker.
- *Play schema + mirror helper* — **✅ landed.** `src/data/playbook/types.ts` defines a play as a named role-assignment (2–4 roles → matchday slot) carrying attack-oriented, mark-relative run-line waypoints + a timed `pass`/`dummy`/`receive`/`carry` schedule + trigger (phases/channels/minSpaceWide) + abort conditions. `src/engine/spatial/playGeometry.ts` (`playPointToPitch`) is the single mirror transform shared by the editor and (future) overlay engine — one definition plays in either direction / off either touchline, no hand-mirrored data.
- *Initial library* — **🟡 5 of ~10 landed.** `src/data/playbook/index.ts`: switch/scissors, miss-2 + blocker, loop, crash + tip-on, dummy switch. Remaining: miss-1, back-door screen, blindside strike off scrum, midfield bust off lineout, 1-3-3-1 same-way pattern.
- *Play editor* (§ 9) — **✅ landed.** A fourth Phase Animator mode: author run-line waypoints + timing per role on a draggable mark, flip attack direction to preview the mirroring, import any shipped play, export play JSON → `src/data/playbook/`. Round-trips the library byte-for-byte; `scripts/checkPlaybookSeed.ts` (in `verify`) fails CI if the editor's embedded seed drifts from the library.
- *Overlay engine* (`PlayOverlay.ts`) — **🟡 mechanism landed DARK.** `createPlayOverlay`/`driveOverlayTick`/`evaluatePlayAborts`/`couplePlayBall` bind roles → agents (the play's carry role → the rng-chosen carrier slot; others → their authored slot), install the run-line waypoints as the Layer-1 source for the bound agents, run the play's pass schedule, and evaluate per-tick aborts (`receiver_covered`/`intercept_risk`/`turnover`) that revert the carrier to a forward corridor run with no teleport. Layers 2–3 (contact, the abort checks) stay live; one driver per agent (the re-anchors run first, the overlay overwrites its bound roles last). Wired into `runCarrySim` behind `input.play?` — **undefined in live play (no selection yet) so the carry is byte-identical: silent-score golden + § 13 bands unchanged.** Scenarios drive it directly (left↔right / first↔second-half mirror-identical trajectories + abort integrity, `checkSpatialScenarios.ts`). Constants in `balance/spatialDecision.ts` (`PLAY_OVERLAY`). Selection (a later commit) populates `input.play` and re-baselines.
- *Spatial FirstPhase* — **✅ landed.** FirstPhase joined `SPATIAL_PHASES`: a strike off a scrum/lineout resolves its carry through the spatial substrate (same hybrid template as PhasePlay — legacy `rng()` seam preserved, spatial line-break/contact verdict overrides), the World cold-seeds on FirstPhase and persists into the following Breakdown, and the authored choreography still drives the live display (spatial `frames` ride along for WP8). Outcome-affecting → golden + bands **re-baselined**. Set-defence calibration uses first-phase-LOCAL levers (`EVASION.firstPhaseDefenderBonus = 18` — the square set line is harder to beat 1-on-1; `GAP_BREAK.firstPhaseBreakMetresBonus = 8` — a strike break carries to the line, lifting first-phase tries and cutting downfield breakdowns) + two breakdown knobs (`ruckRetentionBonus 9→11`, `notRollingAwayBasePct 4→2.6`); every § 13 band holds (post-merge 5-seed: pts 25.57, tries 3.94, pen 12.44, tackAtt 63.17, tackMade 61.10, TO 2.16, home-win 52.89). This is what lets the overlay's FirstPhase strike plays change outcomes (its run lines feed the spatial verdict).
- *Selection + defensive familiarity* — **✅ landed (PhasePlay + FirstPhase).** `src/engine/playSelection.ts`: `selectPlay()` rolls a fire gate (`PLAY_SELECTION.firePct = 9`) on the OUTCOME stream, filters `PLAYBOOK` by trigger, and picks weighted-random by `styleAffinity` (the team's effective `attackingStyle` IS the "default playbook" — no stored per-team list) × `(1 − familiarityWeightDrop × recency)`. **Both `handlePhasePlay` and `handleFirstPhase`** call it on every spatial carry (FirstPhase reads `!goCrashBall` as the wide call — crash ball is a tight/mid strike, out-the-back the wide play): a hit emits `PLAY_SELECTED` (bumps `state.playRecency[side][playId]`, decays the side's others — a new `MatchEvent` variant through `applyMatchEvent`; the match is transient so no save bump) and overlays the carry. **Familiarity bites BOTH** (owner call): recency lowers the selection weight (variety) AND widens the overlay's abort radii (`PLAY_OVERLAY.familiarityReadGain = 0.4` — defenders read a repeated move). Tuned **neutral** to the owner's post-rebalance § 13 targets (5-seed tries 5.55, pts 36.17 — both in band); only the silent-score golden regenerated (`791f6a2f…`). Two new scenarios guard the familiarity levers (read + decline; 33 total). Constants in `balance/spatialDecision.ts` (`PLAY_SELECTION`).
- *Match-day playbook UI* (§ 7.2 tier 2) — **⬜ outstanding.** `PreMatchScreen` picker (~4–6 plays weighted by channel) + a tactics-modal playbook tab over the existing `ui:tacticsChange` → `TACTICS_UPDATED` seam. Sonnet.
- *Scenarios* — **⬜ outstanding.** miss-2 vs fold speed; abort integrity; familiarity decline; left↔right / first↔second-half mirroring; play-abort-rate gate.
