...
| B | `MatchCoordinator.tick` sees `phase === KickAtGoal`, calls `KickAtGoalHandler.advance()`. Rolls the goal kick (`resolveGoalKick`), applies `CONVERSION_KICKED` / `PENALTY_GOAL_KICKED`, emits a 2-step `[kicker_compose, success | miss | kick_for_goal]` event that `CommentaryFeed` stagger-reveals one `lineGap` apart. Applies `POSSESSION_SWAPPED` + `BALL_REPOSITIONED { x: 50, y: 50 }` + `KICK_AT_GOAL_RESOLVED` + `PHASE_CHANGED` to `KickOff`. | **Frozen** (`ClockController.advanceMinute` returns 0 when `phase === KickAtGoal`) |

The inter-tick delay between A and B is shorter than `tickDelayMs` so the build-up doesn't burn a full sim tick. `MatchCoordinator.nextTickDelay()` returns `clamp(300, 1200, tickDelayMs × 0.6)` when `phase === KickAtGoal`. At 1× (2500 ms tick) the build-up is 1500 ms; at 4× (400 ms tick) it floors at 300 ms; at ½× (5000 ms tick) it caps at 1200 ms. The resolve event's two lines then reveal one `lineGap` apart (`tickDelayMs × 0.46`).
