...
| `WEEK_ADVANCED` | After all fixtures of a round are applied, via `runWeeklyTick` | `calendar.week += event.weeks ?? 1` — `calendar.week` is now a **monotonic week counter** (elapsed weeks since the season opened), no longer the league-round index. `weeks` lets a single event span a multi-week gap (an international break advances by the whole-week gap to the next fixture). Does **not** touch `calendar.date` — `runWeeklyTick` re-homes the date afterwards. Also prunes aged-out mid-season FA rejection cooldowns (genuinely week-based). The "which league round" reads use the derived `leagueRound(state)` helper (`src/game/leagueRound.ts` = player's completed league rounds + 1), not `calendar.week`. |
| `MATCHDAY_ADVANCED` | (a) After a cup / European matchday's result is recorded; (b) by `runWeeklyTick` / `fromSave` to re-home the date onto the next league round | Sets `calendar.date = toDate` and nothing else — `calendar.week` is untouched. Consumes no RNG; runs none of `WEEK_ADVANCED`'s weekly passes. For the cup/European path the matchday drains in the post-match `afterTraining` tail before the Hub re-surfaces the next league-round CTA. (Step 3 of the F-2 migration: cup/European still use date-only `MATCHDAY_ADVANCED` and do NOT yet tick the monotonic week — that arrives in step 4.) |
