...
| `AISubstitutionDirector.ts` | Pure (no-RNG) module owned by `MatchCoordinator`. Called once per tick after `AITacticalDirector`. From `AI_SUBS_VALUES.earliestSubMinute` (50') onwards, swaps any AI starter at or below `fatigueThreshold` (60%) for a like-for-like bench replacement — exact position match first, then forward/back group. Bulk subs are allowed (loops until no eligible candidate remains). Routes swaps through `MatchCoordinator.substitute` so the commentary + mutation path is identical to a manager sub. Tuning in `balance/aiSubs.ts`; full breakdown in [Substitutions](#substitutions). |
...
**Presenter pacing & run-ahead (step 4).** `CommentaryStreamer` is the match presenter — a beat buffer (`Beat = { event, display }`) drained at a single steady wall-clock gap `beatGap = tickDelayMs × COMMENTARY_PACING.beatGapFraction` (`balance/commentary.ts`; default 0.6, calibrated against the measured ~1.63 beats/tick so total match duration ≈ the pre-decoupling wall-time). The producer (`MatchCoordinator.tickBody`) no longer waits `tickDelayMs` between ticks in live mode — it reschedules ASAP and is throttled only by a run-ahead gate at the top of `tickBody`: while `streamer.bufferDepth() ≥ COMMENTARY_PACING.lookaheadBeats` it re-checks at `beatGap` instead of producing. This is a **poll**, never a wait on a presenter promise, so it cannot deadlock; silent fixtures skip it and run flat-out (keeping the existing `tickDelay` schedule, so determinism is untouched). The producer can never run past a human-decision tick — the penalty / kick-off / forced-sub modals `await streamer.flush(...)` (drain the cushion to the present at `beatGap`) then block on the user inside the same tick, before the next tick is scheduled. Half-time and full-time drain-then-stop the same way. `CommentaryFeed`'s multi-step reveal (try build-up, TMO, direct cards) staggers at `tickDelayMs × COMMENTARY_PACING.stepGapFraction` (refreshed on `ui:speedChange`) so it scales with the speed slider in step with the beat cadence. **Note:** live pacing is not exercised by `npm run verify` (silent path) — cadence feel and the run-ahead gate need a browser playtest.
...
**Scope:** The managed (human) team substitutes via the UI. AI-controlled teams are driven by `AISubstitutionDirector` (`src/engine/AISubstitutionDirector.ts`), a pure (no-RNG) sibling of `AITacticalDirector` called once per tick after `fatigue.tick()` and before `resolvePhase()`. From `AI_SUBS_VALUES.earliestSubMinute` (50') onwards it loops over each AI side and, while the bench still has a like-for-like replacement, swaps the most-fatigued on-field player whose `fatiguePct` sits at or below `AI_SUBS_VALUES.fatigueThreshold` (60%). Replacement preference is exact position match → forward/back group; no "any bench" fallback (a wing on for a prop weakens the scrum more than a 60% prop staying on). Bulk subs are allowed — real coaches do clear the bench in a single window around 50–55'. The director routes each swap through `MatchCoordinator.substitute()` so the mutation seam + commentary emit + silent-mode gate are identical to a manager sub. In silent (headless) fixtures both sides adapt; in a live match the human side is never touched.
