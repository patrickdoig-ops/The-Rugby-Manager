...
| `PREM_CUP_KNOCKOUT_RECORDED` | Per knockout match — player's (`recordPlayerCupKnockoutResult`) or headless (`simDueCupKnockouts`) | Records the result (incl. `playerSide`); cascades SF winners → final slots (SF1→home, SF2→away), final winner → `championTeamId`. Mirrors `PLAYOFF_RESULT_RECORDED`. |
