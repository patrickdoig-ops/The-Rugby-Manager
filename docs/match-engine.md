...
**Line model + fold (`balance/spatialShape.ts`).** `DEFENSIVE_LINE` maps the tactic to lateral `slotSpacing` + `standOff` + the per-tick re-anchor weights `forwardPress` + `lateralTrack` (blitz 5.5/2.0/0.85/0.35 tight, flat, rushes up; drift 7.5/4.5/0.35/0.85 wide, deep, shepherds laterally; hybrid 6.5/3.0/0.6/0.6). `LINE_SLOT_COUNT = 12` front-line slots are laid out centre-out around the mark; the nearest defender to each slot is assigned greedily **in slot order** (ruck channel + inside first — the cover-the-threat-then-the-width priority). **Density gradient (shape-realism):** the slots are NOT evenly spaced — `slotSpacing` is now the gradient SCALE, and each per-side slot N sits at the cumulative sum of gaps that grow from `DEFENCE_SPACING.innerRatio · slotSpacing` (tight at the ruck — guards + tight forwards) to `outerRatio · slotSpacing` (wider toward the edge — backs marking wide), over `spreadSlots` (innerRatio 0.72, outerRatio 1.35, spreadSlots 4; ≈ 4.7 m inner → 8.8 m outer on hybrid). The ratios are chosen so the average gap ≈ the old uniform spacing, keeping the line's extent ~unchanged while redistributing it into the gradient. **Open-side spread (WP 5):** at a WIDE ruck the centre-out slots that would clamp against the near touchline are redistributed to the OPEN side instead of packing the line — up to `LINE_OPEN_REDIRECT_CAP = 2` of them (keeping 1–2 blindside pillars and bounding the outcome shift; home-win-share rises with the redirect count). The gradient applies at every ruck (a central ruck is no longer byte-identical to the old uniform layout — the shape-realism change re-baselined the golden by design, holding every § 13 band). The line sets up `standOff` in FRONT of the mark (toward the attackers, `mark.x + attackDir·standOff`). `BACKFIELD_COUNT` (one_back 1, else 2) posts **deep behind the line toward the DEFENDING try line** at `mark.x + attackDir·BACKFIELD_DEPTH` (`BACKFIELD_DEPTH = 22`) — `attackDir` points toward the line the attackers attack (the defenders' goal), so the backfield sits on the defenders' side, never stranded offside on the attackers' side (WP 2 Bug ① — the sign was inverted, posting the back three at `mark.x − attackDir·22` in front of the attackers). **Pitch-centred backfield (shape-realism):** the two deep defenders split around the **y=50 midline** (`y = 50 ± BACKFIELD_PITCH_SPLIT`, split 16 → ~34/66), covering the WHOLE width regardless of the ruck's lateral position — a wide ruck no longer strands both deep defenders on the near touchline with the far side open (a single backfielder sits on the midline). Replaces the old ruck-centred `BACKFIELD_SPREAD`. **Backfield defenders are selected by matchday slot, not by current depth:** `solveDefence` picks the back three — fullback (slot 15) first, then wings (14, 11) — using a fixed preference order so kick-cover is always the correct specialist players. Fallback: if a back-three slot is absent (carded side, `role: 'empty'`), the next back-three slot is tried; then any remaining backs (slots 9–15) sorted deepest-first; and finally the legacy depth sort over all remaining defenders. Props and forwards stay in the front line. Fold speed is `deriveFoldSpeedMult(stamina, positioning, fatigue)` (`balance/spatialSteering.ts`: weights 0.6/0.4, multiplier 0.45→1.0 × the fatigue curve) written onto the agent's `speedScale`, which `MovementSystem` applies to the arrive() speed — **slow folds leave the overlap** (the emergent payoff). `CARRY_CORRIDOR_TICKS = 22` micro-ticks per beat.
...
- **`reanchorDefence(roles, carrier, p)`** (the `preMove` hook) re-anchors every front-line slot onto the LIVE carrier each tick. **Cover defence (shape-realism):** the deep backfield normally holds for kick cover, but once the carrier has broken PAST the line (`(carrier.x − targetLineX)·attackDir > COVER_DEFENCE.triggerAdvance = 2`) the NEAREST backfielder (by lateral channel) steps UP to the cover point — `(carrier.x + attackDir·leadAhead, carrier.y)`, `leadAhead = 3`, so he meets the carrier square + goal-side (a Layer-3 REACT) — while the other holds deep; whether the break is shut down is emergent (`ContactSystem` resolves the cover tackle; a fast carrier still beats it). The line presses UP toward the gain line, `targetLineX = openingLineX + attackDir·min(pressCap, carrierAdvance·pressGain·forwardPress)` (`DEFENCE_REANCHOR.pressGain = 0.55`, `pressCap = 9.0`), and each slot's lateral target slides toward the carrier's channel, `y = slotY + (carrier.y − slotY)·lateralTrack·trackGain` (`trackGain = 0.12`). `pressCap` bounds the forward creep so the line cannot rush past the carrier and the offside sweep (measured after the loop against the FIXED mark) stays coherent. `MovementSystem`'s top-speed clamp still applies, so a pressing slot accelerates onto the new target — it never teleports. Result: the front line moves ~3.5–4.7 u/beat (was ~1.5–2.8 u static), visibly advancing/folding as the carrier runs.
...
- **`createPlayOverlay(world, params, play, familiarity)`** binds role names → agents. The play's **carrier role** (the one that performs a `carry` action — the strike runner) binds to the handler's rng-chosen `carrierSlot`, so the verdict geometry is the one the play set up; every other role binds to its authored slot. Returns `null` when the carrier can't bind (a carded `role: 'empty'` slot — the carry then runs plain); a non-carrier role on an empty slot is dropped (the play degrades, never glitches). The opening ball-holder is the role whose first non-`receive` action is earliest (the feeder); a `pass` action schedules a hand-over at `action.t + PASS_CHAIN.flightTicks`. `familiarity` (0..1, the defence's recency for this play) is stashed on the overlay state for the abort scaling below.
...
- **`evaluatePlayAborts(ov, world, params, t)`** — per-tick, deterministic geometry (no rng): `receiver_covered` (a defender within `PLAY_OVERLAY.receiverCoverRadius = 3.2` of the next receiver), `intercept_risk` (a defender within `interceptLaneRadius = 2.4` of the live pass lane), `turnover` (the carrier carrying into a defender within `turnoverRadius = 2.6` while isolated — no bound support nearer than `isolationRadius = 9.0`). All three radii are scaled by `(1 + familiarityReadGain × familiarity)` (`familiarityReadGain = 0.4`) so a **read** play (high recency) has its abort windows widened — defenders react faster and the repeated move dies more often (at familiarity 1 the radii grow 40%; at 0 they are unscaled). On any → the overlay aborts: the carrier is handed a fresh forward corridor target from his CURRENT position (continuous — `MovementSystem`'s accel cap makes the target switch a velocity ramp, never a teleport), and from that tick the ShapeSolver re-anchors are the only writer for the bound roles again (`driveOverlayTick` becomes a no-op). Contact is suppressed (`carrierHasBall`) until the strike runner actually has the ball — a defender reaching a decoy is not a tackle. Constants in `balance/spatialDecision.ts` (`PLAY_OVERLAY`); the run lines themselves are content, not tuning.

**Selection (`src/engine/playSelection.ts`).** `selectPlay({ phase, channel, spaceWide, style, recency })` decides — on the **outcome** rng stream, so it re-baselines — whether a carry runs a play and which one. It rolls a fire gate (`PLAY_SELECTION.firePct = 9` — a set move is the exception, the rest run the plain ShapeSolver carry), filters `PLAYBOOK` by trigger (phase / channel / `minSpaceWide`), then weights each eligible play by `styleAffinity[style][channel]` (the team's effective `attackingStyle` IS the "default playbook" — `wide_wide` favours wide-channel plays, `keep_it_tight` the close channels) × `(1 − familiarityWeightDrop × recency)` (a stale play keeps 35% of its weight at recency 1), and picks weighted-random. `selectChannel(goWide, spaceWide)` maps the WP5 wide-vs-hard call to the play channel (hard → `tight`; wide → `wide` if `spaceWide ≥ wideChannelSpace = 24`, else `mid`). **Both `handlePhasePlay` (`OpenPlayEvent`) and `handleFirstPhase` (`FirstPhaseEvent`) call it on every spatial carry** — FirstPhase passes `phase: 'FirstPhase'` and reads `!goCrashBall` as the wide call, so a crash ball is a tight/mid strike and out-the-back is the wide play. A hit emits `PLAY_SELECTED` (the recency bump) and passes `input.play` + the pre-bump `input.playFamiliarity` into `runCarrySim`; `state.playRecency` is the per-match, per-side, per-play recency map (`PLAY_SELECTED` reducer: decay the side's plays by `recencyDecay = 0.7`, bump the chosen one by `recencyBump = 0.5`, clamp `[0,1]`; never persisted — the match is transient). Tuned **neutral** to the owner's post-rebalance § 13 targets (5-seed tries 5.55, pts 36.17 — both in band), so adding selection regenerates only the silent-score golden, not the bands. Scenarios (`checkSpatialScenarios.ts`) drive the overlay directly via the scenario kit: left↔right and first↔second-half **mirror-identical trajectories**, **abort integrity** (a covered strike receiver aborts within a few ticks with no position discontinuity), the **familiarity read** (a defender just outside the fresh cover radius kills a play only once it is read), and **familiarity decline** (a repeated play is selected materially less).
...
**Cover-beat run-on (line-break → try conversion).** A clean line break that falls SHORT of the try line on its carry metres is not dead: the carrier races the last line of defence — the covering fullback/winger — to the line. After the metres-based `isTryScoredAt` check fails for a `line_break`, each carry handler (`OpenPlayEvent`, `FirstPhaseEvent`, `KickReturnEvent`) rolls `breaksCover(distToLine)` (`FieldPosition.ts`). On a hit the break runs on: the carry gain is extended to the line and the beat scores (no cover tackler is credited). The probability is **distance-graded** — `P = COVER_BEAT.baseBeatPct(88) × max(0, 1 − distToLine / COVER_BEAT.decayRange(55))` (`balance/openPlay.ts`), where `distToLine` is the metres from the projected break end to the try line: near the line a single covering defender is easily beaten; the further back the break, the more defenders converge, decaying to zero by 55m out. This is the lever that turns line breaks into tries — most breaks happen 22m+ out where the metres draw alone rarely reaches, so without the run-on conversion sits at ~16%; the run-on lifts it to the real-world ~34% (measured profile: ~89% inside 10m, ~56% from 22-40m, ~13% from 60m+), which carries open-play tries to ~5.5/match. It leaves the line-break RATE and tackle-completion % untouched (the cover-beat only re-routes a break that already happened from "stopped short → Breakdown" to "ran on → try"). One outcome-stream `rng` draw per short-of-line break.
...
| ≥ −14.5 | `turnover` → PhasePlay (possession flips) |
| < −14.5 | `penalty_defending` → Penalty (possession flips to defending team) |

(`turnoverMargin` was widened `−14 → −14.5` with the shape-realism attacking changes — the full-width forward structure pulled the chronically-tight turnovers band to its floor, and the wider open outlet left home-win headroom to absorb the small turnover lift.)
