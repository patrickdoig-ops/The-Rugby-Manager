...
| `PROMISE_BROKEN` | `checkTransferRequestsAndPromises()` when `toRound` has passed with insufficient starts | Clears `Player.playingTimePromise`; fires `PLAYER_MORALE_ADJUSTED` with `delta = MORALE.promiseBrokenPenalty (−15)`. No forced sale. |
