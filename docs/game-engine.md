...
| `INTERNATIONAL_RELEASE_OFFERED` | `GameCoordinator.resolveInternationalWindow` (autumn break only, once per season) and on the player's release/refuse action | Sets `state.player.internationalReleaseDecision` to `'pending'` (inbox card shows) or `'decided'` (card removed). Cleared at `SEASON_ROLLED_OVER`. |
