...
| `src/engine/spatial/CarrySim.ts` (WP 2, continuity WP 4) | `seedWorld(world, params)`: the cold-World formation snap (three solvers + `seedFormation`) — called once by `handlePhasePlay` on cold entry, before any branch (see "World continuity"). `runCarrySim(world, state, input)`: takes the **persistent** World (owned by `MatchCoordinator`), runs `solveDefence` + `solveCarryCorridor` + `solveAttackSpread` + the micro-ticks (with the `preMove` re-anchor + `postMove` ball-couple hooks), returns the plain-data verdict `{ lineBreak, tacklerSlot, spatialMetres, offsideOffenderSlot, frames }`. It does NOT seed — the World is already in shape (cold beat seeded by `seedWorld`; continuation beat carries positions over), so agents run from their current positions and nothing teleports. |
