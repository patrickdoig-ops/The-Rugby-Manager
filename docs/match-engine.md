...
- **Pass phase (`runPassPhase`, WP 5; `balance/spatialDecision.ts`)** prefixes a live carry: the ball sweeps from the ruck (scrum-half) through the backline (`PASS_CHAIN.flightTicks = 3` per pass) to the carrier at his receiving point, BEFORE he runs — so a wide play visibly moves the ball across the backline (a back carrier sweeps ≈ 13 m laterally; a forward takes a direct pass at the ruck, ≈ 0). The chain comes from `OpenPlayEvent` (`[scrumHalf, flyHalf, ballCarrier]` for a wide play, `[scrumHalf, ballCarrier]` for a hard carry). It is **frame-ONLY**: it snapshots the upstream chain members it reposts, captures the sweep frames, then RESTORES them + the ball — so the carry runs from byte-identical state and **live == silent** (headless league sims stay in lockstep; the pass OUTCOME — knock-on / interception — stays on the legacy `rng()` rolls in `OpenPlayEvent`, not re-rolled here). Skipped entirely on the silent path. Only the 1-2 upstream chain dots reset at the pass→carry seam (a within-beat visual the WP 8 renderer will interpolate).
