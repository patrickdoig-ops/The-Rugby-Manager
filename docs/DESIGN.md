...
- **`OnboardingDirector.ts`** — reactive orchestrator. Subscribes to `screenRouter.onScreenShow` and renders the first not-yet-passed step whose `screen` matches the one that appeared, after a ~300ms settle (show-observers fire *before* the screen is un-hidden + slid in). `advanceClick` steps attach a delegated **capture-phase** click listener (fires before any `stopPropagation`; the advance is deferred a frame so the triggering click resolves first). Navigation stays in `main.ts`: the director only triggers the first jump (Home → team select) via the `onStartTour` callback passed to `initOnboarding()`; every later step is reached by the player acting on the real UI.
...
**Flow it walks.** *Phase 1 (get into the first match fast):* Home intro → team-selector → team-info → mode-picker (Quick Start) → **Hub: next-match card first** (know your opponent) → **the “?” help button** (where to find per-screen guidance if stuck) → **Squad** (squad-management walkthrough: select a player **by their number square** → swap → expand a card for quick attributes → open a full player profile → save) → Hub (Continue) → pre-match → **live match** (switch into the 2D pitch view for kickoff → the other views → the sim controls; non-blocking, never covers the pitch) → match-result. Tactics are deliberately **not** in the pre-match leg — they also surface in the build-up, so the tour skips straight to kickoff. *Phase 2 (season management), continuing straight on from the first result:* Hub → **Tactics** (the advanced/optional game-plan screens, now that they've played) → **Club → Assistant Manager** (how to delegate League Cup ties) → **Club → Staff** (hire your backroom) → **Contracts & Transfers → Transfers** (sign a free agent) → **Contracts & Transfers → Loans** (send a youngster out on loan to develop) → finish. The whole thing is one continuous, resumable tour (the `rugbyOnboardingStep` pointer spans both phases); the final step is the Loans card. A step may set `returnToHub` so its button also navigates home (`onGoHub` callback) — used after the deep Staff step to spare a two-level menu back-out.
