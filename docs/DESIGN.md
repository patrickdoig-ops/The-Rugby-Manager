...
**Flow it walks.** *Phase 1 (first match):* Home intro → team-selector → team-info → mode-picker (Quick Start) → **Hub: next-match card first** (know your opponent) → **the “?” help button** (where to find per-screen guidance if stuck) → **Squad** (squad-management walkthrough: select a player → swap → expand a card for quick attributes → open a full player profile → save) → **Tactics** → Hub (Continue) → pre-match → **live match** (a non-blocking card on the sim controls explaining auto-play + speed; never covers the pitch) → match-result. *Phase 2 (season management), continuing straight on from the first result:* Hub → **Club → Assistant Manager** (how to delegate League Cup ties to the assistant) → **Club → Staff** (hire your backroom) → **Contracts & Transfers → Transfers** (sign a free agent) → **Contracts & Transfers → Loans** (send a youngster out on loan to develop) → finish. The whole thing is one continuous, resumable tour (the `rugbyOnboardingStep` pointer spans both phases); the final step is the Loans card. A step may set `returnToHub` so its button also navigates home (`onGoHub` callback) — used after the deep Staff step to spare a two-level menu back-out.
