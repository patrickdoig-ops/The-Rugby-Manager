...
**Flow it walks:** Home intro → team-selector → team-info → mode-picker (Quick Start) → **Hub: next-match card first** (know your opponent) → **Squad** (squad-management walkthrough: select a player → swap → expand a card for quick attributes → open a full player profile → save) → **Tactics** → Hub (Continue) → pre-match → **live match** (a non-blocking card explaining auto-play + speed controls; no spotlight, never covers the pitch) → match-result (finish).
