...
**Presenter pacing (step 3).** `CommentaryStreamer` is the match presenter — a beat buffer (`Beat = { event, display }`) drained at a single steady cadence. Per beat the gap targets `tickDelayMs / bufferDepth`, clamped to `[tickDelayMs × COMMENTARY_PACING.minGapFraction, tickDelayMs]` (`balance/commentary.ts`). A quiet 1-beat tick shows its line at the tick rate; a multi-event burst drains no tighter than the floor (default ⅓ tick) and any overflow **carries forward** into the next tick's idle window instead of firing several lines in one visual burst — replacing the old reset-every-tick `tickDelayMs / N` spacing. Deeper buffer → smaller gap (down to the floor) → faster catch-up, so the backlog stays bounded while the producer still drives one tick per `tickDelayMs` (no run-ahead yet; that is step 4, which fills this same buffer). `CommentaryFeed`'s multi-step reveal (try build-up, TMO, direct cards) now staggers at `tickDelayMs × COMMENTARY_PACING.stepGapFraction` (refreshed on `ui:speedChange`) instead of a fixed 500ms, so both pacing layers scale coherently with the speed slider.
