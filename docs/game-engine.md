...
**Cup / European matchday weeks are single weeks.** The Autumn Nations / Six Nations breaks are no longer one big block — they run as a sequence of per-matchday League Cup game-weeks (and European matchdays sit weekly inside the league calendar). Each of those matchdays is its own game-week, so its post-match training screen renders a **single** "Training Week" card, not the whole multi-week break. `main.ts::afterMatchdayTraining` passes an explicit `gap` to `showTrainingPostMatch` — `{ weeks: 1, days: GameCoordinator.cupMatchdayGap().days }` for the cup (the span to the next matchday, via `upcomingGapFromDate(calendar.date, nextMatchdayDate())`) and `{ weeks: 1, days: 7 }` for European — overriding the league-round `upcomingGap` the screen would otherwise derive (which, keyed off the derived `leagueRound`, would report the surrounding multi-week break and mislabel the screen a "Training Block"). The injected `runBlock` applies via `runCupMatchdayTraining` / `runEuropeanMatchdayTraining` over that single span.
...
**Weekly flow integration.** `maybePlayEuropeanFixture(onDone)` in `main.ts` is the entry point. It first checks `getCurrentEuropeanFixture()` (plays match if present), then checks `getCurrentEuropeanRound()` (shows round/final screen if present), then calls `onDone`. Both branches recurse — so a week with both a player match and pending AI-only rounds fully drains before returning to the Hub. After each round screen is dismissed, `markEuropeanRoundShown` and autosave are called. **Each player European matchday is a full game week:** the match result is followed by its own training week (`afterMatchdayTraining` → `GameCoordinator.runEuropeanMatchdayTraining`, a fixed 7-day recovery/dev week — European keeps its league-driven `calendar.date`, so a date-derived gap isn't available) before the flow recurses. `checkSeasonDeterminism` drives this path (`drainEuropean`), so the European play-out + training are determinism-harnessed.
