...
- **Season-scope mutations have their own boundary: `applySeasonEvent(state, event)`** in `src/game/applySeasonEvent.ts`, operating on `GameState` (`src/types/gameState.ts`). `GameCoordinator` is the only caller. `SeasonEvent` variants are domain-meaningful and the same `default: const _: never = event;` exhaustiveness contract applies. The current set:
  ...
  - **Career layer (Phase 1 + 2 of the transfer-system roadmap, live on main)**: `ROSTER_SEEDED` (one-shot at `newSeason` / v4-save migration; populates `state.career.roster` keyed by `rosterId`), `PLAYER_SEASON_STATS_ACCUMULATED` (post-fixture aggregator into `Player.seasonStats`), `PLAYER_AGED` (per-stat deltas at rollover via `careerRollover.computeRollover`), `PLAYER_RETIRED` (rosterId removed from `ClubState.squad`; Player record retained for archive), `SEASON_ROLLED_OVER` (composite — archives standings + awards, resets results / standings / per-player seasonStats, regenerates fixtures with synthetic Sept–May dates).
  - Adding contract renewals, transfer offers, free-agent flows, or academy / persona generation (Phases 3-7 in `docs/transfer-system.md`) means new variants on this union — not new mutation seams.
...
**Navigation flow.** Home → Team Selector → Hub → PreMatch → Match → Result → Round Results → League Table → Hub. The Hub (`src/ui/HubScreen.ts`) is the in-season control centre: six tiles route to Fixtures (`fixture-list`), League (`league-table`), Contracts (`contracts`, live as of v2.23a — read-only squad / wage / expiry / marquee view), and three remaining placeholder destinations (Squad / Training / Transfers — no-op handlers until those screens exist), plus a Settings cog and a "Go to next match" CTA. The post-match chain walks the manager through Round Results (`RoundResultsScreen` — every fixture of the just-completed round, player's match highlighted) then League Table (in a forward "Continue → Hub" mode set via `showLeagueTablePostMatch(onContinue)`) before landing on Hub. **When the final round of a season finishes, the chain detours**: LeagueTable → EndOfSeasonScreen (final standings + top scorer + MVP) → RolloverScreen (retirements + per-player aging deltas) → Hub for the new season. Detection is via the `game:seasonComplete` flag latched in `main.ts`; `rollSeason()` runs on the EndOfSeason → Rollover transition and returns the applied `SeasonEvent[]` for the screen to render. PreMatch back and the in-season sub-screen back-buttons all return to Hub. Settings has two entry points with different back targets — `goSettingsFromHome` (back → Home) and `goSettingsFromHub` (back → Hub) in `main.ts`.
...
**Career & roster (Phases 1 + 2, live as of v2.23a).** `GameState.career` (`src/types/gameState.ts`) holds the persistent senior-squad roster + per-club squad pointers + archived season standings, so the league can span multiple years. Three load-bearing distinctions:
...
**Contracts (Phase 2 — read-only).** Every roster Player carries `PlayerContract { clubId, expiresOn, annualWage, isMarquee }` + `reputation: number`, seeded once via `src/game/contractSeeder.ts::seedContractFields` from `rngTransfer` (wage = `WAGE_BY_RATING` × `POSITION_SCARCITY` × `WAGE_NOISE`, length age-banded, expiry staggered ~22/38/42 across +1/+2/+3 years). One hand-authored marquee per club via a `Marquee: yes.` annotation in `docs/team-data.md`, translated by `scripts/generateTeamJsons.mjs` into a `contract: { isMarquee: true }` JSON override that `contractSeeder` honours. Surfaced read-only on `ContractsScreen` (Hub → Contracts); cap pill is dimmed (no enforcement until Phase 3). Tuning constants in `src/engine/balance/transfers.ts`. **Roadmap for renewals / cap enforcement / free agency / Reg 7 poaching / persona generation: `docs/transfer-system.md`** (Phases 3-7).

**Save schema.** `SAVE_VERSION = 6` (as of v2.23a). v5 saves auto-migrate by running `contractSeeder` over the persisted roster on load; v4 saves trigger a full fresh roster seed; v3/v2 use the original schedule-only restore; v1 discarded. Full version table in `docs/game-engine.md` § "Save format".
...
### Transfer system (Phases 3-7)
- **Salary cap enforcement + interactive marquee designation, end-of-season renewals, free-agent signings, cross-Prem poaching (Reg 7), generated player supply** are all sketched in **`docs/transfer-system.md`** with concrete file-by-file work items per phase. Phases 1 + 2 are live (multi-season rollover + read-only contracts); the seams left for Phases 3-7 are the same `applySeasonEvent` mutation boundary and the `rngTransfer` stream — no new architectural primitives needed.
