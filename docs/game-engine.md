...
|---|---|
| `GameCoordinator.ts` | Public API (`newSeason`, `fromSave`, `getState`, `getCurrentFixture`, `recordPlayerMatchResult`, `seedPlayoffBracket`, `getPlayerPlayoffMatch`, `recordPlayerPlayoffResult`, `simulatePendingPlayoffMatches`, `rollSeason`, `toSavePayload`, `setSquadStatus`, plus the market-window methods listed below, plus the loan/transfer-request methods: `makePlayingTimePromise`, `grantTransferRequest`, `rejectTransferRequest`, `loanOutPlayer`, `recallLoanedPlayer`, `signLoanPlayer`, `releaseLoanPlayer`). **Calendar-block surface (unified-calendar migration — partially wired; see `docs/unified-calendar-gap-analysis.md`):** `getNextBlock(): CalendarBlock | null` — thin wrapper over `calendarBlocks.nextBlock(state, allTeamIds)` where `allTeamIds` is sourced from `[...this.teamsById.keys()]` (the same pattern used by `rollSeason`). **`getNextBlock` is wired into the Hub CTA preview + `main.ts`'s `onContinue`/`playBlock` dispatch.** `simRestOfBlock(block): Promise<void>` — sims all non-player, not-yet-recorded fixtures in the block: iterates distinct league rounds in the block (ascending) calling the private `simLeagueRound` per round; calls `simDueCupFixtures` for any cup fixtures; calls `advanceEuropeanCompetitions` for European fixtures; calls `simulatePendingPlayoffMatches('sf')` then `simulatePendingPlayoffMatches('final')` for playoff fixtures. Competition order (league → cup → european → playoff) is fixed so RNG call order is stable. **`simRestOfBlock` is built but not yet on the driver path — `playBlock` still dispatches to the per-competition sims by priority (commit 4 in the gap analysis wires it in).** Neither method drives calendar advance or weekly passes — those flow through the elapsed-week seam below. Owns the `GameState`. The "tick" of the game engine is a player match completing: `recordPlayerMatchResult` applies the player's score, headlessly simulates the other fixtures of the round, runs `seasonStatsCollector` over each, then advances the week via the private **`runWeeklyTick()`** — the league-round weekly tick. **The weekly passes are split into two seams (F-2 step 4):** (a) **`tickElapsedWeeks(toDate)`** — the *competition-agnostic, elapsed-week* seam, and **forward-only**: it advances `calendar.date` and ticks weeks ONLY when `toDate > calendar.date`. A `toDate` at or before the cursor ticks **0** weeks and leaves `calendar.date` untouched (no backward `MATCHDAY_ADVANCED`). On a forward advance it sets `calendar.date = toDate` via `MATCHDAY_ADVANCED`, fires one `WEEK_ADVANCED { weeks }` where `weeks = round(daysBetween(currentDate, toDate)/7)` (min 1), then runs `advanceEuropeanCompetitions` + morale decay × `weeks` + scouting-accuracy gain × `weeks`, and emits `game:weekAdvanced`. It is called from **every** competition's advance — `runWeeklyTick` (league), `advanceMatchdayCalendar` (cup / European / playoff matchdays), and `resolveInternationalWindow` (the tail segment between the last cup matchday and the resuming league round). Because the cursor only ever moves forward and each advance counts only the strictly-later days since its current position, summed over a season the `WEEK_ADVANCED` total equals the true calendar span (first→last fixture, ≈38 weeks for 2025-26) with no week double-counted and none skipped — including across international breaks and on bye weeks. (b) **`runWeeklyTick()`** — the *league-round-keyed* seam: advances the cursor only as far as the next actual calendar event — `getNextBlock()?.startDate` (the next unplayed block's start across ALL competitions), falling back to `earliestDateForRound(fixtures, leagueRound(state))` then `+SEASON_VALUES.weekLengthDays` — then calls `tickElapsedWeeks(newDate)`. During an international break the next event is the first cup/European matchday, so the league round ticks only up to it and the break's own matchdays tick forward through the rest; re-homing straight to the next *league* round (the former behaviour) would jump the cursor past the whole break and force the break's matchdays to move it backward — the H1 double-count bug (audit measured week 61 vs the true ≈38), now fixed. For a pure league-to-league gap with no intervening matchday the next block IS the next league round, so behaviour there is unchanged; the block start date exists regardless of whether the player has a fixture, so byes still advance the cursor correctly. It then runs the passes that are NOT safe to fire per-calendar-week or per-cup-matchday: transfer-request/promise checks, poach-threat assessment, and the AI early-renewal cadence (their streak edges / cadence counters are per-league-round). The **injury-recovery tick stays in `recordPlayerMatchResult`** (looped `upcomingGap().weeks` — the full prev-league→this-league gap, league-only) and the playoff path (`recordPlayerPlayoffResult`, one week per match), so injuries heal exactly once per elapsed week with no double-heal. Once the last R18 fixture is recorded `recordPlayerMatchResult` also seeds the playoff bracket and emits `game:bracketSeeded`; `game:seasonComplete` then fires later, after the League final resolves. |
...
| `careerRollover.ts` | `computeRollover(state, allTeamIds)` — pure module; given current GameState, produces the SeasonEvent stream for a rollover, in this order: (0) `TRANSFER_ACTIVATED` for every pending Reg 7 pre-agreement (stable rosterId-ascending), (1+2) per-roster aging via `AGE_CURVES` + `STAT_NOISE` Gaussian noise + retirement check against `RETIREMENT_CURVE` (growth is scaled by `proximityMultiplier × appearancesMultiplier` and floored at 0; decline fires at full rate regardless), (3) Phase 7 supply — `ACADEMY_GRADUATED` 2-4 per club (stable alpha club id order) + `FOREIGN_IMPORT_ARRIVED` 5-10 single batch, both consuming `generatePersona`. Each academy graduate is generated into the club's **neediest position** (biggest shortfall vs `POSITION_FLOORS`, via `squadComposition.neediestPosition`; null → the random roll) so intake closes per-position gaps instead of starving the scarce single-label positions (Lock/Prop/Hooker/SH/FH) under uniform-random generation — the random position roll is consumed regardless, so the `rngTransfer` offset is unchanged. (3b) squad-size floor — project each club's post-rollover squad size from the queued events (+ loan-in releases) and emit extra `ACADEMY_GRADUATED` (also position-targeted) to top any club below `MIN_SQUAD_SIZE` (35) up to the floor (stable alpha order, after the regular intake; a no-op for healthy squads), (3c) squad-size cap — for any club projected above `MAX_SQUAD_SIZE` (45), emit `CONTRACT_TERMINATED('released')` for its lowest-OVR non-marquee / non-loan players (excluding those retiring or moving this rollover) down to the cap, never cutting a position already at its `POSITION_FLOOR` (so the over-deep back row is trimmed, the scarce positions protected); RNG-free (pure OVR sort, stable by rosterId) so it doesn't shift the stream, (4) awards (top tries, MVP by avg rating with `mvpMinAppearances` floor) + composite `SEASON_ROLLED_OVER` with regenerated fixtures (Sept-May synthetic weekly dates, skips Nov + Feb). |
| `personaGenerator.ts` | Phase 7 — `generatePersona(seed, calendarDate)` produces a deterministic `Player` from `rngTransfer`: name from `NAME_POOLS` per nationality (English / Welsh / Scottish / Irish / French / South Africa / NZ / Australia / Fiji / Argentina, ~15-20 first + last names each); nationality biased by `NATIONALITY_BY_CLUB` for academy grads, uniform for foreign imports; position uniform across 12 generic positions; dob anchored to season-open year ± `ageBand`; stats `targetOverall ± uniform(12)` per stat, clamped 1-99; wage £20k fixed for academy (RPA rookie rate) or `WAGE_BY_RATING × POSITION_SCARCITY` rounded to £5k for imports; 2-year deal default. |
...
| `seasonFixtures.ts` | Pure. `collectSeasonFixtures(state, playerTeamId): SeasonFixtureRow[]` — the player's whole-season schedule across every competition, BOTH played (carrying `result: { homeScore, awayScore }`) and upcoming, chronologically sorted. Sibling to `collectUnplayed` (mirrors its per-competition traversal) but includes played fixtures + filters to the player; knockout/play-off matches with unseeded (null) sides are excluded. `SeasonFixtureRow = BlockFixtureRef & { opponentId, playerIsHome, played, result? }`. Consumed by `SeasonFixturesScreen` (Competitions → Fixture List). Team identity is resolved in the screen, not here. |
| `stageLabel.ts` | Pure competition/stage labelling over a `BlockFixtureRef` — one source of truth shared by the inbox's next-opponent line and `SeasonFixturesScreen`. `stageLabelLong` reproduces the inbox's combined competition+stage string verbatim (regression-safe); `competitionLabel` (full name, distinguishes European Cup vs Shield), `competitionTag` (LGE/CUP/EUR/P-O badge), `competitionAccentClass` (`fl-comp--*`), `stageNameLong` ("Round 7"/"Pool"/"Round of 16"/…), and `stageBadge` ("7"/"P"/"R16"/"QF"/"SF"/"F") serve the screen. |
...
| `balance/career.ts` | Rollover tuning — `AGE_CURVES` per stat (peakAge / growth / decline — growth rates halved vs v1, physical decline steepened, mental decline flattened), `STAT_NOISE` (Gaussian std-dev + clamp), `RETIREMENT_CURVE` (forwards / backs cumulative probabilities), `SEASON_AWARDS.mvpMinAppearances`. Also holds the development-ceiling constants: `POTENTIAL_HEADROOM` (age-banded OVR headroom ranges seeded once at game-start), `PROXIMITY_CURVE` (piecewise-linear headroom→multiplier table, applied to growth only), `APPEARANCES_CURVE` (step function of season appearances applied to rollover growth). Exports `proximityMultiplier(potential, ovr)` and `appearancesMultiplier(apps)` helpers consumed by both `careerRollover` and `trainingWeek`. Also holds the roster-hygiene constants: `ARCHIVE_CAP` (15 — retained-archive depth; the live and saved archive are both trimmed to this, and retired roster records outside the retained reference set are pruned at rollover) and the squad-size band `MIN_SQUAD_SIZE` (35) / `MAX_SQUAD_SIZE` (45) plus `POSITION_FLOORS` (per-position depth targets summing to 33; drive position-targeted academy intake and the size-cap release — see `src/game/squadComposition.ts`). |
...
| `WEEK_ADVANCED` | Inside `tickElapsedWeeks` (the elapsed-week seam), driven by `runWeeklyTick` (league round), `advanceMatchdayCalendar` (cup / European / playoff matchday), and `resolveInternationalWindow` (international break tail — the final segment from the last cup matchday to the resuming league round) | `calendar.week += event.weeks ?? 1` — `calendar.week` is now a **monotonic week counter** (elapsed weeks since the season opened), no longer the league-round index. `weeks` lets a single event span a multi-week gap (an international break advances by the whole-week gap to the next fixture). Does **not** touch `calendar.date` — `runWeeklyTick` re-homes the date afterwards. Also prunes aged-out mid-season FA rejection cooldowns (genuinely week-based). The "which league round" reads use the derived `leagueRound(state)` helper (`src/game/leagueRound.ts` = player's completed league rounds + 1), not `calendar.week`. |
| `MATCHDAY_ADVANCED` | Inside `tickElapsedWeeks` (the only emitter): called by `advanceMatchdayCalendar` (cup / European / playoff matchday), `runWeeklyTick` (league), and `resolveInternationalWindow` (international break tail); also by `fromSave` to re-home the date onto the next league round | Sets `calendar.date = toDate` and nothing else — `calendar.week` is bumped by the accompanying `WEEK_ADVANCED` in `tickElapsedWeeks`, not here. Consumes no RNG. `tickElapsedWeeks` only emits this event on a strictly-forward advance (`toDate > calendar.date`), so the cursor never moves backward. (F-2 step 4: cup / European / playoff matchdays now route through `advanceMatchdayCalendar` → `tickElapsedWeeks`, so they tick the monotonic week + morale/scouting/European-catch-up passes for the weeks they advance — season progression is competition-agnostic, no longer batched at the next league round.) |
...
| `PLAYER_RETIRED` | Per retiring player per rollover | Removes `rosterId` from `ClubState.squad`, `career.freeAgents`, `career.pendingMoves`, and `career.loanPool`, and sets `Player.retired = true` (the rollover aging loop and weekly morale decay skip flagged players). The `Player` record is retained in `state.career.roster` only while something still references it (retained-archive leaders / per-player history, captain, scouting); once unreferenced it is dropped by the retired-roster prune inside `SEASON_ROLLED_OVER`, so the roster stays bounded across a long career. |
...
| `PLAYOFF_RESULT_RECORDED` | One per playoff match — player (via `recordPlayerPlayoffResult`) or AI sim (via `simulatePendingPlayoffMatches`) | Sets `result` on the named match (incl. optional `kickWinner`). Cascades: on a SF result, populates the Final's matching slot from the SF winner (SF1 → home, SF2 → away). On the Final's result, sets `championTeamId`. The advancing side is resolved by `knockoutWinnerId` (`src/game/knockoutWinner.ts`): higher score, or — on a level score (extra time finished level) — `result.kickWinner`. Does NOT touch `league.standings` — playoffs are independent of league points. |
...
| Event | When | Effect |
|---|---|---|
| `PREM_CUP_SEEDED` | Once per season — `newSeason` (fixed 2025-26 pools) + appended **last** in `computeRollover` (year 2+, redrawn pools) | Builds `state.league.premCup` — two pools (each with `zeroStanding` rows) + 40 pool fixtures (3 legs: 4/pool pre-season, 6/pool autumn, 10/pool six-nations). Idempotent on a matching `seasonLabel`. |
| `PREM_CUP_FIXTURE_RECORDED` | Per pool fixture — the player's live/assistant-simmed match (`recordPlayerCupPoolResult`) or a headless sim of the rest of the matchday (`simRestOfCupLeg`) | Sets the fixture result (incl. `playerSide` — the manager's side, or null for a headless sim) + applies it to that pool's standings via the shared `applyResultToStanding` (`leagueTable.ts`) (same 4/2/0 + bonus rules as the league). |
...
| `PREM_CUP_KNOCKOUT_RECORDED` | Per knockout match — player's (`recordPlayerCupKnockoutResult`) or headless (`simDueCupKnockouts`) | Records the result (incl. `playerSide` + optional `kickWinner`); cascades SF winners → final slots (SF1→home, SF2→away), final winner → `championTeamId`, all via `knockoutWinnerId`. Mirrors `PLAYOFF_RESULT_RECORDED`. |
| `PREM_CUP_ROUND_SHOWN` | After the player dismisses a cup leg-results recap in the weekly flow | Appends the round key (`leg:0`-`leg:2`; leg 2's recap includes the knockout bracket, gated on the champion being crowned) to `premCup.shownRounds` so it doesn't re-surface. Mirrors `EUROPEAN_ROUND_SHOWN`. |
| `PREM_CUP_FEATURED_ADDED` | Per cup fixture recorded (the featured rosterIds); `reset: true` after the leg's nudge fires | Accumulates the rosterIds who featured in the current leg onto `premCup.legFeatured` (persisted → reload-safe), or clears it. Drives the once-per-leg development nudge fired at leg completion by `maybeFireCupLegDevelopment`. |
| `PLAYER_CUP_DIRECTION_SET` | When the user toggles the cup direction | Persists `state.player.cupDirection` (`'best'` \| `'rest_first_15'`). |
| `PLAYER_CUP_MANAGE_LIVE_SET` | When the user toggles "manage my cup matches" vs "let the assistant take over" | Persists `state.player.cupManageLive` (boolean). Remembered default, flippable per block / matchday. Additive-optional save field (`cupManageLive`, omitted when false) — no `SAVE_VERSION` bump. |
...
| `STAFF_POOL_SEEDED` | `GameCoordinator.newSeason` (season 1 start, `generateStaffPool(1)`) and `careerRollover.computeRollover` (year 2+, carries forward hired staff + generates a fresh free pool). Quick-start path additionally pre-hires one assistant manager, fitness coach, and scout (all rating 65, `clubId` set to the player's team) by prepending them to the pool before the event fires. | Sets `career.staff = event.staff` and `career.nextStaffId = event.nextStaffId`. IDs use the `nextStaffId` counter (mirrors `nextRosterId`). Free-pool entries have `clubId: null`; hired entries carry the managed-club id. Both `staff?` and `nextStaffId?` are additive-optional — absent on legacy saves (treated as empty pool, no staff effects). |
...
| `PLAYING_TIME_PROMISED` | `GameCoordinator.makePlayingTimePromise(rosterId)` — inbox "Promise game time" CTA | Sets `Player.playingTimePromise = { toRound: leagueRound(state) + 5, startsRequired: 3, startsAtPromise: seasonStats.starts }` (`PLAYING_TIME_PROMISE` in `balance/transfers.ts`); clears `wantsTransfer`. `toRound` is a league round; checked each weekly tick against `leagueRound(state)` — if passed and the player hasn't accumulated `startsRequired` starts since the promise, fires `PROMISE_BROKEN`. |
...
| `LOAN_PLAYER_SIGNED` | `GameCoordinator.signLoanPlayer(rosterId)` — LoanScreen "Sign on loan" button | Adds `rosterId` to the managed club's `squad`; sets `Player.loanIn = { fromRound: leagueRound(state) }`; removes from `career.loanPool`. Player is immediately available for selection and trains normally. |
...
| `BOARD_CONFIDENCE_ADJUSTED` | Per human result in `recordPlayerMatchResult` (`applyBoardResult`) | `confidence = clamp(0, 100, confidence + delta)`. Per-result delta keyed on `expectedToWin`: win-as-favourite `+3`, win-as-underdog `+6`, draw `∓2`, loss-as-favourite `−6`, loss-as-underdog `−3`; a third straight league loss adds `−5`. Also fired by `applyBoardResult` when `fanSentiment < 30` (delta `−1`, reason `fan_sentiment_low`). |
...
| `FAN_SENTIMENT_UPDATED` | Per human league match in `applyBoardResult` (via `BoardCoordinator`) | `state.player.fanSentiment = clamp(0, 100, current + delta)`. Default 50 when absent on legacy saves. Base delta: win `+2`, loss `−2`, draw `+1`; doubled for derby fixtures; +0.5 extra on a win with attacking/expansive tactics (`wide_wide` style or `possession` game plan). Surfaced on `BoardConfidenceScreen` (label: Poor / Steady / Good / Excellent). Feeds `computeAttendance` as a multiplier: `0.9 + sentiment/500` (50 → ×1.0, 100 → ×1.1, 0 → ×0.9). Additive-optional save field (`GameState.player.fanSentiment`) — no `SAVE_VERSION` bump. |

The **Club page** (`ClubMenuScreen`, reached from the Hub's Club tile) is the home of board confidence: a summary band + 0–100 meter and a live **"what's driving it"** breakdown from `boardConfidenceFactors(state)` (season league objective vs current position, the **European objective** (`board.europeanObjective` phrased via `europeanObjectiveText` — e.g. "win the European Cup" / "reach the European Shield semi-finals"), recent results, winning/losing runs, formal-warning latch). The European objective is set by `BoardCoordinator.seedEuropeanObjective` (year-1 from `boardAmbition`, else from the prior league finish) and judged through the season by `applyEuropeanElimination`; the week-1 owner's-message inbox item now also states it. The recurring **owner's messages** in the inbox (`inbox.ts`: week-1 objectives + the round-6 / round-11 mid-season block reports) additionally close with an owner's-voice read-out of the confidence band (`ownerConfidenceLine` → `confidenceBand`), so the manager tracks their standing in narrative form too.
...
`(playerTeamId, rootSeed)` plus the player's series of results produces an identical final league table + roster `baseStats` + retirement list on every run, across multiple seasons. Verified by `scripts/checkSeasonDeterminism.ts` — runs three full seasons with `rollSeason()` between each, drives the League Cup live weekly flow (`drainCupBreak`: per-matchday cup sims via `getCupBreakStep` / `runPlayerCupFixtureHeadless` / `simDueCupFixtures`, a per-matchday `runCupMatchdayTraining`, and call-up/return bracketing via `beginInternationalBreak` / `resolveInternationalWindow` — at the pre-season block + rounds 6 / 11), drives the player's European fixtures + per-matchday training (`drainEuropean`: `getCurrentEuropeanFixture` / `recordPlayerEuropean*` / `runEuropeanMatchdayTraining`) so both competitions play out to a champion (pool play-out is incremental via `advanceEuropeanCompetitions`), exercises both the renewal and signing windows (AI-only, no user decisions) between each pair, snapshots per-season standings + results hash + the full SeasonEvent stream + the renewal + signing offer hashes + the Prem Cup pools/fixtures/bracket + both European competitions (pools, fixture results, knockout cascade + champion) + post-window free-agents pool + final-state roster baseStats + seasonsCompleted, asserts byte-equal SHA-256 on a second run. A **third run** exercises the save/load contract: after the season-2 → season-3 rollover, the coordinator is serialised via `toSavePayload()`, JSON round-tripped, rebuilt via `GameCoordinator.fromSave`, and season 3 finishes on the restored coordinator — its hash must match the uninterrupted run (this is what enforces the `careerRngOffset` load/reload determinism claim). Season snapshots are frozen by value at capture time (live references would alias across the coordinator swap). `npm run verify` runs both the match-level harness (2 seeds × 2 pairings) and the career-level harness; both must pass before commit.

**Silent-fixture golden scores.** `scripts/checkSilentScores.ts` (also in `npm run verify`) pins the *absolute* outcomes of the headless AI-fixture path (`simulateFixture`). It sims a fixed 49-fixture set (one-way round-robin + a few derby / neutral-venue / fill-rate flagged fixtures) from a pinned root seed and asserts the full result set (each scoreline + both teams' `MatchSnapshot` summary aggregates) hashes to a checked-in golden. Unlike `checkDeterminism` (which drives *live* coordinators) and `checkSeasonDeterminism` (which only compares two runs of the *same* code), this catches a silent-path optimisation that changes simulation results — it is the regression guard for any "faster headless sim, identical outcomes" change. Regenerate the golden (paste the printed `actual` into `GOLDEN`) only when an outcome change is intentional.
...
Tuning constants live in `RENEWAL` (`src/engine/balance/transfers.ts`): `loyaltyDiscount` (current club discount on demanded wage), `aiTargetCapUtilisation` (under-cap clubs aim for this fraction of their own salaryBudget), `aiReleaseRatingFloor` (below this OVR, expiring players are at risk regardless of cap headroom).
...
- **Recovery tick**: `tickInjuryEvents(state, gapStartIso?)` (`src/game/injuryEffects.ts`) runs at the start of `recordPlayerMatchResult` (after the re-entry guard, before any new fixture is recorded). Pure RNG-free walk over `state.career.roster`; emits `INJURY_TICK_ADVANCED` for every injured player and `PLAYER_RECOVERED` for any whose counter hits zero. It's looped `upcomingGap(state).weeks` times — the number of whole weeks between the player's previous and upcoming match (`src/game/trainingCalendar.ts`), so a long international window (≈8 weeks across the Six Nations break) heals proportionally more than a normal 1-week turnaround. A 6-day and 8-day gap both round to one week. The injury tick stays league-only (not in the elapsed-week `tickElapsedWeeks` seam) precisely so it heals the full prev→this gap exactly once — the cup/European matchday advances that fire `tickElapsedWeeks` during a break tick the week counter + morale + scouting but NOT injuries, so there is no double-heal. Order: tick × N → record fixtures → roll new injuries → `runWeeklyTick()` (re-home date + `tickElapsedWeeks` → `WEEK_ADVANCED` + elapsed-week passes, then the league-round-keyed passes).
...
- **Calibration**: ~1.87 injuries / match across both teams at `INJURY.basePctPerTackle = 6.0`. Tuning in `src/engine/balance/injuries.ts`; calibration target band is 1.8-2.2.
...
**Week-at-a-time blocks (league gaps only).** The gap until the player's next *league* match is rarely exactly 7 days — normal turnarounds run 6-9 days (Fri/Sat/Sun kick-offs). The post-match league screen renders one card per *training week* of the gap (`upcomingGap(state).weeks = round(days/7)`, min 1), each with its own intensity; forwards/backs focus is shared across the block. The real day count is split into ~7-day periods by `splitGapIntoPeriods` (`src/game/trainingCalendar.ts`). **Condition recovers per day** (so an 8-day gap recovers more than a 6-day one at equal intensity), while development and injury rolls fire once per period (per week). A short single-week turnaround (≤6 days) defaults to Light with an advisory banner. These defaults are advisory only — the manager can override every week.

**Cup / European matchday weeks are single weeks.** The Autumn Nations / Six Nations breaks are no longer one big block — they run as a sequence of per-matchday League Cup game-weeks (and European matchdays sit weekly inside the league calendar). Each of those matchdays is its own game-week, so its post-match training screen renders a **single** "Training Week" card, not the whole multi-week break. `main.ts::afterMatchdayTraining` passes an explicit `gap` to `showTrainingPostMatch` — `{ weeks: 1, days: GameCoordinator.cupMatchdayGap().days }` for the cup and `{ weeks: 1, days: GameCoordinator.europeanMatchdayGap().days }` for European — overriding the league-round `upcomingGap` the screen would otherwise derive (which, keyed off the derived `leagueRound`, would report the surrounding multi-week break and mislabel the screen a "Training Block"). Both delegate to the private `GameCoordinator.matchdayGap()`: `upcomingGapFromDate(from, nextPlayableDate(state, playerId, from))` where **`from` is `lastPlayedMatchdayDate(state, playerId)`** — the date of the matchday the manager just played (the latest dated, resolved cup OR European fixture across pool + knockout), falling back to `calendar.date` only when nothing has been played. The just-played date is the correct anchor because a non-league fixture often kicks off on a Friday, whereas `calendar.date` tracks the upcoming **league** round's earliest fixture (frequently a later date): anchoring on `calendar.date` understated the rest gap — e.g. a Fri cup match (Sep 19) with the next league round opening on a later Friday (Sep 25) reported a 3-day gap when the player's own next match was 9 days away (Sep 28), so a rested squad was wrongly advised (#118). The injected `runBlock` applies via `runCupMatchdayTraining` / `runEuropeanMatchdayTraining`, which split the same `matchdayGap()` span across the plan's periods — so the displayed days always equal the span the engine recovers over.
...
- **Effects (split across `beginInternationalBreak` + `resolveInternationalWindow` on `InternationalBreakCoordinator`, `src/game/InternationalBreakCoordinator.ts` — `GameCoordinator` delegates and then wraps with post-return hooks).** `beginInternationalBreak` (RNG-free, called at break entry) runs `selectInternationalSquads` → emits `PLAYER_CALLED_UP` per player (sets the transient `internationalDuty` flag). The flagging is guarded against a re-entry double-bump of `internationalCaps` by `anyOnInternationalDuty(window)` — keyed off whether any player is already flagged for the window, not whether a fresh re-selection overlaps the flagged set (the selection can drift after a cup-dev nudge, the flags can't); on re-entry it returns the flag-derived call-ups (`callUpsFromDutyFlags`) so the UI matches who was actually called up. The players stay flagged across the break's cup game-weeks — `computeTrainingWeek` skips `internationalDuty` players exactly like injured ones (no club condition recovery / development while away) and `selectionUnavailableIds` keeps them out of the live cup XV. At the end of the cup weeks (`getCupBreakStep === 'resolve_returns'`), `GameCoordinator.resolveInternationalWindow` first calls `tickElapsedWeeks(upcomingLeagueDate())` to advance the calendar to the resuming league round and tick the final break segment's elapsed-week passes (morale decay, scouting accuracy, week counter), then delegates to `InternationalBreakCoordinator.resolveInternationalWindow` which reconstructs the call-ups from the still-flagged players (`callUpsFromDutyFlags` — reload-safe, immune to a mid-break OVR drift), runs `resolveInternationalBreak` → per call-up (rosterId-asc) rolls `minutesPct` from rank, a reduced return `condition` (full-load ~55-70), an injury chance (`~8-12%` × minutesPct, reusing `INJURY_SEVERITY`), **camp training stat boosts** (see below), and — England heavy-load only — a `restObligation`; emits `PLAYER_TRAINED` (camp gains) then `PLAYER_RETURNED_FROM_DUTY` (+ `PLAYER_INJURED`). Returns an `InternationalBreakSummary` shown on the **International Break screen** (`src/ui/InternationalBreakScreen.ts`). **After the return summary is built, `GameCoordinator.resolveInternationalWindow` publishes 1–2 media inbox stories** via `generateInternationalStories` (seeded from `hashSeed(state.seed, 'intl', window, week)` — keyed to the career stream but deterministic, no `rngTransfer` calls): story 1 focuses on the standout or injured player (if any); story 2 is a general "players back from duty" note for squads with 3+ call-ups (50% chance).
...
**Weekly tick.** `StaffCoordinator.advanceScoutingAccuracy()` (`src/game/StaffCoordinator.ts`) runs once per elapsed week inside `tickElapsedWeeks`, so it accrues on cup / European / playoff matchday weeks too, not just league rounds. For every entry with a live `assignedScoutId` pointing at a currently-hired scout, it emits `SCOUTING_ACCURACY_ADVANCED { rosterId, delta: scoutWeeklyGain(scout.rating) }`.
...
The League Cup plays out as **ordinary game weeks** during the pre-season block + the two international breaks. The manager either plays their own matches live (`cupManageLive`) or hands each block to the assistant; every other club is simmed headless. Two pools of 5, full home-&-away double round-robin split into leg 0 (pre-season) / leg 1 (Autumn) / leg 2 (Six Nations), top two per pool → semis → final. Self-contained: drains condition + a once-per-leg featured-player development nudge, but no budget/cap/reputation effect and cup stats stay out of league leaderboards. State lives on `state.league.premCup` (`PremCupState`); the weekly flow is driven from the Hub's cup CTA (`onPlayCupStep` in `main.ts`) through `GameCoordinator.getCupBreakStep` / the `InternationalBreakCoordinator` cursor + record methods; scheduling + future-season pool redraw in `src/game/cupScheduler.ts`; tuning in `src/engine/balance/premCup.ts`. **Full breakdown in `docs/league-cup.md`.**
...
`EuropeanCoordinator.seedEuropeanComps(seasonLabel)` (called from `newSeason()` and `rollSeason()`) emits `EUROPEAN_COMP_SEEDED` for both competitions, initialising `EuropeanCompState` with pools, zeroStandings, and all fixtures — **no results**. The competitions are **not** pre-populated; results accumulate over the season.

**Incremental play-out (`GameCoordinator.advanceEuropeanCompetitions`).** Driven from every elapsed-week tick (inside `tickElapsedWeeks`, so on cup / European / playoff matchday weeks as well as league rounds) and after the player records a European result. It sims the **AI** fixtures whose date has arrived (`runPoolStage(comp, asOf)` / `runKnockoutStage(comp, asOf)` — both date-gated, skipping the player's own team), seeds each knockout once its pool completes, and sims due knockout matches round-by-round (the cascade fills the next round's slots in the reducer). The player plays their own fixtures live (`maybePlayEuropeanFixture`). So both pool standings and knockout results fill in week by week, mirroring the real calendar. **Ordering rule:** the Shield knockout seeds only once **both** pools are complete — the Shield R16 seeds the 5th-placed EC teams as drop-downs, so the EC pool's final standings must be settled first.

`runPoolStage(competition, asOfDate?)` runs headless for all unplayed fixtures **except** the player's own (and, with `asOfDate`, only those whose date has arrived). Each AI fixture calls `simulatePoolFixture`, which derives a `MatchCoordinator` seed from the per-round seed constant (timing-independent — the result is the same whenever it's simmed), runs a silent match, and emits `EUROPEAN_FIXTURE_RECORDED { …, playerSide: null }`. The reducer updates pool standings via the standard bonus-point logic (4pts win / 2pts draw / 0pts loss + try bonus + losing bonus).
...
**Knockout extra time.** Every knockout fixture — playoff, Prem Cup SF/final, and European R16→final, live or headless — is built with `allowExtraTime: true` (passed to `simulateFixture` / `MatchCoordinator`; pool fixtures are not). A draw at full time plays two 10-minute extra-time periods, then a kicking competition if still level (see `docs/match-engine.md` § "Extra time"). When the kicking competition decides it, the match score stays level and the advancing side is carried on `result.kickWinner` (from `state.engine.extraTimeWinner`); `knockoutWinnerId` resolves the winner from score-then-`kickWinner`, so there is no home-side tiebreak.
...
**Weekly flow integration.** `maybePlayEuropeanFixture(onDone)` in `main.ts` is the entry point. It first checks `getCurrentEuropeanFixture()` (plays match if present), then checks `getCurrentEuropeanRound()` (shows round/final screen if present), then calls `onDone`. Both branches recurse — so a week with both a player match and pending AI-only rounds fully drains before returning to the Hub. After each round screen is dismissed, `markEuropeanRoundShown` and autosave are called. **Each player European matchday is a full game week:** the match result is followed by its own training week (`afterMatchdayTraining` → `GameCoordinator.runEuropeanMatchdayTraining`, a recovery/dev week scoped to `matchdayGap()` — the span from the just-played European fixture date to the player's next matchday) before the flow recurses. `checkSeasonDeterminism` drives this path (`drainEuropean`), so the European play-out + training are determinism-harnessed.
...
| `EUROPEAN_KNOCKOUT_RECORDED` | `competition, stage, matchIndex, homeScore, awayScore, homeTries, awayTries, kickWinner?` | Writes result on the bracket match; advances the bracket (winner via `knockoutWinnerId` — score, or `kickWinner` on a level score after extra time) |
...
**Forward compatibility.** `parseSavedGame` routes a **lower, known** version through an ordered `MIGRATIONS` pipeline (`MIGRATIONS[N]` upgrades vN→v(N+1); empty at v1) so a `SAVE_VERSION` bump carries existing careers forward instead of orphaning them. A future/garbage version (or a gap in the migration chain) is rejected cleanly. `scripts/checkSaveSchema.ts` (run by `npm run verify`) boots a fresh deterministic career, serialises it, and fails if the `SavedSeason`/`SavedCareer` key set or `SAVE_VERSION` drifts from its pinned snapshot — so a shape change can't ship without a conscious bump (or a snapshot update for an additive-optional field).
...
`GameState.player.fanSentiment?: number` is an additive-optional field (no bump). Absent on legacy saves → treated as 50 (neutral) by all consumers. Persisted directly on `SavedSeason` when non-default (restored inline from the save payload via `BOARD_STATE_SEEDED` path in `fromSave`). Reset each season to 50 by the `SEASON_INITIALIZED` / `SEASON_ROLLED_OVER` flow. See `BoardConfidenceScreen` for display and `computeAttendance` for the multiplier formula.

`SavedCareer.hallOfFame?: HallOfFameEntry[]` is an additive-optional field (no bump). Players inducted into the managed club's Hall of Fame at retirement are persisted directly in `toSavePayload` and restored via `CAREER_ARCHIVE_RESTORED.hallOfFame` in `buildCareerArchiveRestoredEvent`. Absent on legacy saves → `state.career.hallOfFame` stays `undefined` (treated as empty by `ClubHistoryScreen`). Induction fires in the `PLAYER_RETIRED` branch of `applySeasonEvent` when a retiring player at the managed club has ≥50 career appearances OR ≥20 career tries across `state.career.archive.playerSeasonHistory`.

**On load** (`GameCoordinator.fromSave`). `src/game/saveMigration.ts` houses two event-payload builders (`buildRosterSeededEvent`, `buildCareerArchiveRestoredEvent`) that normalise a saved career into the `SeasonEvent` payloads replayed by `fromSave` through `applySeasonEvent`. The persisted career state always flows through `CAREER_ARCHIVE_RESTORED` (with optional `freeAgents` / `market` / `pendingMoves` / `teamSeasonStats` / `preSeasonStep` / `playoffs` / `takeoverHistory` / `midseasonRejections` / `activePoachedIds` / `hallOfFame` fields) so every `state.career.*` write stays inside `applySeasonEvent`. The `training` field lives on `state.player`; it's restored via `PLAYER_TRAINING_PLAN_SET` during `fromSave`, mirroring the `PLAYER_TACTICS_SET` / `PLAYER_MATCHDAY_SQUAD_SET` restore path.

`save.currentWeek` is `calendar.week` — now a **monotonic week counter** (F-2 step 3). The shape is unchanged (still a number), so no `SAVE_VERSION` bump. `fromSave` replays `results` (round order), then loops `WEEK_ADVANCED` (one week at a time) until `calendar.week === save.currentWeek` to restore the counter, then re-homes `calendar.date` onto the next league round's earliest kick-off via `MATCHDAY_ADVANCED` (`earliestDateForRound(fixtures, leagueRound(state))`) — `calendar.date` is derived, never persisted, exactly as before (when the old round-indexed `WEEK_ADVANCED` owned the date during this loop).
...
| `medical` | Any squad player still on a 2025 B&I Lions post-tour stand-down (`leagueRound(state) < lionsReturnRound`) — lists returnees, reduced condition, and the round they're available from | `squad` |
...
| `match` | Scout report (priority 110) on the player's **actual next match** in any competition — resolved via `resolveNextOpponent(state, allTeams)` → `nextBlock` (the same calendar-block model as the Hub "Continue" cycle), not just the next league round. Subject reads `Scout report — {stageLabel} vs {opp}` where `stageLabel` is `Round N` / `League Cup[ Final/Semi-Final]` / `European Cup[ Knockout]` / `European Shield[ Knockout]` / `Play-off {Final/Semi-Final}`. Tactical-identity insights fire for any opponent that carries `suggestedTactics` (Premiership clubs from `allTeams`, foreign European clubs from `src/data/european-teams.ts`); the season-stat / standings / player-threat observations fire only for Premiership opponents (`hasLeagueData`, gated on `matchesPlayed ≥ 2`). Suppressed on a bye week (no player fixture in the block). | `fixtures` |
...
| `suspension.forRound === leagueRound(state)` | `disc:suspended:{season}:{rid}` | 80 | "Go to Squad" deep-link |

Counsel advice is active while `leagueRound(state) <= disciplineAdvice.expiresAfterRound`; once it expires the concern item re-appears and can be renewed.
