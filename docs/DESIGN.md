...
- **Club** — second `.ti-tiles.ti-tiles-club` row holding up to four identity tiles: **Founded** (year + `N years old` foot), **Nickname**, **Stadium capacity** (locale-formatted number, stadium name foot), **Head coach** (parenthetical role notes are stripped). Forced 2×2 grid (`grid-template-columns: 1fr 1fr`) so the layout stays stable at any width. All four values use a single body-font 20px bold style scoped to `.ti-tiles-club .ti-tile-value` so numeric and text tiles look uniform. Any tile whose value is missing is omitted; the section disappears entirely if all four are blank.
