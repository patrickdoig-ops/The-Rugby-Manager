...
3. Drives `FatigueAccumulator.tick(timeAdvance)` (`src/engine/FatigueAccumulator.ts`): accumulates elapsed time and, once the accumulator reaches 5 game minutes, calls the pure `computeFatigue(team, elapsedMinutes)` on both teams (home first, then away — order matters because both consume the outcome RNG stream) and emits a `FATIGUE_APPLIED` event for every update. `computeFatigue` also returns newly-fatigued players (crossing below 50%); these are **buffered** and their commentary is emitted only at the next natural break (`flush()`), never mid-open-play.
...
| `src/engine/spatial/ShapeSolver.ts` (WP 2) | Layer-1 ROLE solver, DEFENCE half: `solveDefence` (line slots anchored at the mark, fold via `speedScale`, backfield, returns line roles), `reanchorDefence` (the per-tick re-anchor — see "Per-tick line re-anchor" below), `solveCarryCorridor` (carrier + support pod targets; marks them `role: 'corridor'`), `solveAttackSpread` (placeholder positions for the remaining attackers — forwards in a loose cluster behind the mark, backs fanning out toward the open side with width + depth), `detectGap` (lateral-dominant gap → line-break verdict + nearest line defender), `detectOffside` (creep → offside offender). Assigns destinations + `speedScale` only — never moves anyone (position seeding is `seedFormation`; per-tick movement is `MovementSystem`). |
| `src/engine/spatial/CarrySim.ts` (WP 2) | `runCarrySim(state, input)`: builds the World on PhasePlay entry, runs `solveDefence` + `solveCarryCorridor` + `solveAttackSpread` + `seedFormation` (snap to shape) + the micro-ticks (with the `preMove` re-anchor + `postMove` ball-couple hooks), returns the plain-data verdict `{ lineBreak, tacklerSlot, spatialMetres, offsideOffenderSlot, frames }`. Discards the World on return (cross-phase persistence is WP 4). |
...
**Line model + fold (`balance/spatialShape.ts`).** `DEFENSIVE_LINE` maps the tactic to lateral `slotSpacing` + `standOff` + the per-tick re-anchor weights `forwardPress` + `lateralTrack` (blitz 5.5/2.0/0.85/0.35 tight, flat, rushes up; drift 7.5/4.5/0.35/0.85 wide, deep, shepherds laterally; hybrid 6.5/3.0/0.6/0.6). `LINE_SLOT_COUNT = 12` front-line slots are laid out around the mark; the nearest defender to each slot is assigned greedily. The line sets up `standOff` in FRONT of the mark (toward the attackers, `mark.x + attackDir·standOff`). `BACKFIELD_COUNT` (one_back 1, else 2) posts **deep behind the line toward the DEFENDING try line** at `mark.x + attackDir·BACKFIELD_DEPTH` (`BACKFIELD_DEPTH = 22`, `BACKFIELD_SPREAD = 18`) — `attackDir` points toward the line the attackers attack (the defenders' goal), so the backfield sits on the defenders' side, never stranded offside on the attackers' side (WP 2 Bug ① — the sign was inverted, posting the back three at `mark.x − attackDir·22` in front of the attackers). Fold speed is `deriveFoldSpeedMult(stamina, positioning, fatigue)` (`balance/spatialSteering.ts`: weights 0.6/0.4, multiplier 0.45→1.0 × the fatigue curve) written onto the agent's `speedScale`, which `MovementSystem` applies to the arrive() speed — **slow folds leave the overlap** (the emergent payoff). `CARRY_CORRIDOR_TICKS = 22` micro-ticks per beat.
