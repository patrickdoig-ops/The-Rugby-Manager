...
**Landed:** WP 0 ✅ · WP 1 ✅ · WP 2 ✅ · WP 3 ✅ · WP 4 ✅ · **WP 5 🚧 in progress** (continuous onside attack shape — increment 1 of the ShapeSolver attack-half: the carrier ENGAGES FROM THE RUCK each beat — snapped to the mark, runs forward — `reanchorSupport` trails the support pod, `reanchorAttack` holds the off-ball forward cluster + backline behind the live gain line every tick; fixes the "support/backline ahead of the ball = offside" the watch test flagged. WP 5 onside-discipline scenario guards it; 21 spatial scenarios total; full 5-seed bands hold: pts 23.58, tries 3.63, TO 2.13, pen 12.53, tackMade 64.9. **Still to do for WP 5:** pods/2-4-2 assignment, the three-layer stack formalisation, pass mechanics (run-onto-ball / pass window / flight), carrier utility AI, even defensive spread, retire `OpenPlayResolver` + `Lateral.ts`, shape editor.)
