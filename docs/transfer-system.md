...
- **Enforcement:** salary-cap manager with audit powers; sanctions escalate from fines (4-point deduction up to £75k breach) to **40-point deduction** for breaches over £250k, up to relegation. Modelled as a hard constraint on **new signings**: a club can never be signed into a breach. The user side enforces it at submit time (`submitBid` rejects a bid that would push `clubBudgetUsage` over `salaryBudget`); the AI/resolver side enforces it as a backstop in `resolveSigningRound` — it tracks each winning club's running wage usage and refuses any winner (FA join, poach, or resolver-routed retention) whose commit would breach the hard `salaryBudget`, since the per-decision AI gates use only a *soft* utilisation target and don't see every commit in the round. The renewal window is a deliberate exception: it lets a club keep its existing squad up to its pre-window wage level (`max(salaryBudget × util, preWindowCap)`) rather than dumping its expiring cohort when the budget tightens — loyalty-discounted renewals converge an over-committed club back under budget over the following seasons.
...
`SAVE_VERSION = 1` in `src/ui/SaveManager.ts`. Bump `SAVE_VERSION` and update `ACCEPTED_VERSIONS` whenever the serialised shape changes in a way that would corrupt an existing save on load. New additive-only optional fields don't require a bump. A bump must also add the matching `MIGRATIONS[N]` step (vN→v(N+1)) in `SaveManager.ts` and update the pinned snapshot in `scripts/checkSaveSchema.ts` (run by `npm run verify`). See `docs/game-engine.md` § "Save format" for the full storage/backup/migration model.
