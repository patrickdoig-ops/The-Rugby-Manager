...
5. **Resolution.** `signingResolver.resolveSigningRound(state)` walks rosterIds in ascending order; for each player with ≥1 pending bid, computes `appealScore(state, bid, player) + wageSatisfaction(bid.annualWage, askingWage)` for each bidder, picks the highest, fires `BID_RESOLVED` per bid + the appropriate contract event for the winner (`CONTRACT_SIGNED` / `PRE_AGREEMENT_SIGNED` / `CONTRACT_EXTENDED`). Ties break by lower `clubId`. **Reservation gate:** if the winning bid's wage is below `askingWage × WAGE_NEGOTIATION.reservationFloorRatio` (0.80) the player holds out — every bid is marked lost, no contract fires, and `SigningOutcome.heldOut` is set (rendered as a "Held out — wage too low" row).
...
**Terms.** `retentionTermsFor(state, rosterId)` — the same loyalty-discounted wage + age-banded length the end-of-season renewal window would generate (`fresh-market × (1 - RENEWAL.loyaltyDiscount)`); this is the asking wage. The user negotiates an `offeredWage` in the wage modal (`offerEarlyRenewal(rosterId, offeredWage?)`), defaulting to asking. Because `retentionTermsFor` advances `rngTransfer`, the modal's asking anchor + read use the **RNG-free** `contractSeeder.estimateMarketWage` (noise-free wage curve) so previewing never perturbs the stream.
