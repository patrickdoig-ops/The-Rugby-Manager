...
**WP 5 status (🟡 core engine landed; remainder categorised):** the WP 5 OBJECTIVE — the open-play attacking game reads as rugby (shape, pods, onside support, the ball moving through the backline) — is delivered by increments 1–5 above and live on main. The remaining WP 5 deliverables, by nature:
- *Carrier utility AI* (§ 5.4) — engine refinement. The carry/pass/kick decision is currently the legacy rng + tactics path (`decideKick` / `goWide`); geometry-based utility scoring (corridor gap, support proximity, composure) is a refinement with calibration risk — not yet done.
