...
When a formation moves between beats, `PitchPlayers` adds `dot-transitioning` to the `#pitch-2d-field` element. This enables `transition: top 0.5s ease, left 0.5s ease` on every `.pitch-dot` simultaneously. The class is removed via a generation-tokened `setTimeout(..., 600)` once the transition completes (the token stops an earlier timer cutting a later glide short at fast tick speeds). Dots are already at their new positions — the CSS transition is triggered by the position change. **The rule is: every phase change glides, except the snap phases** (`KickOff`, `HalfTime`, `FullTime` — the directive's `snap` flag), which get the faster `dot-snap-transition` (400 ms) instead — those reset the whole frame and should read as a cut, not a drift. Because CSS animates from each dot's actual current position, the one rule covers every predecessor (Lineout→Maul, FirstPhase→Breakdown after the `keepLineout` hold, kick→KickReturn after `keepKickFormation`, …) without per-transition cases. `PhasePlay` additionally re-arms the glide on every beat (not just the phase change) so the involved actors ease to their ball-relative spots each carry. Both classes are removed in `reset()`.

**Between-ruck formation drift (PhasePlay).** Without help, the ~27 dots `openPlayLayout` does *not* place stand frozen through a multi-phase passage (only the carrier / tackler / immediate support move). `appendFormationDrift` in `pitchChoreography.ts` fixes this: on every `PhasePlay` beat, after `openPlayLayout`, each not-already-placed on-field dot eases `DRIFT_WEIGHT` (0.4, in `pitchAnimConstants.ts`) of the way from its last committed position toward its slot's target in a **ball-anchored `BREAKDOWN_CLEAN` frame** (the same canonical-frame transform as `placeFormation` — `dir` from attack direction, lateral mirror + paired-slot swap on a single-axis reflection; attackers → `atk`/`clampX`, defenders → `def`/`clampDefenderX`). Because the target re-anchors to the live ball each beat and the dots only move a fraction toward it, the defensive line keeps re-forming goal-side of an advancing ball and the attacking support drifts upfield, converging over several phases without ever snapping. `PitchPlayers` feeds the seed: a `lastPositions: Map<key, {x,y}>` in **game coords**, set whenever a dot's position is committed (not parsed from style strings) and deleted when a dot fades. Drift dots carry **no `from` and no `isCarrier`** (they glide via the per-beat PhasePlay `dot-transitioning`, channel-exclusive with carrier/tackler/chase) and are flagged `isDrift` so they **skip the `dot-moving` pulse** — 27 dots blurring every beat would be far too noisy. The target table is a starting point; an authored `PHASE_PLAY_DRIFT` frame can replace `BREAKDOWN_CLEAN` later without touching the mechanism.
...
- **`onboardingSteps.ts`** — the script as data: an ordered `OnboardingStep[]` (`{ id, screen, title, body, target?, advance, advanceClick?, kind?, cta? }`). `advance: 'action'` steps carry no button — the player advances by tapping the **real, spotlighted UI** (learn-by-doing), either via the screen-change it triggers or, when `advanceClick` (a CSS selector) is set, via a same-screen click on a matching element (used for the squad-management walkthrough: select → swap → expand). `advance: 'next'` steps show a primary button. `kind` flags the opening `'intro'` card (Start tour / Not now) and the closing `'final'` card.
- **`CoachMark.ts`** — the renderer (`.rm-onb-*`, `style/onboarding.css`): a spotlight ring (`.rm-onb-spot`, a box-shadow halo that dims everything except the target) plus an anchored tooltip card. The overlay is `pointer-events:none` **except the card**, so the player can always click the real UI behind it and can never be soft-locked. Card buttons sit in a fixed position — the advance target never darts around the screen. A spotlighted step also runs a rAF-throttled `MutationObserver` so the ring re-measures and stays glued to a target whose node is replaced by an in-place re-render (e.g. squad rows on every selection/swap). An untargeted card defaults to a dimmed centre, but a step may set `placement: 'bottom'` to anchor it at the foot of the screen with **no dim** (so a selection screen like the team list stays visible/tappable), and `dismissible` to add a "Got it" that hides the card without advancing.
- **`OnboardingDirector.ts`** — reactive orchestrator. Subscribes to `screenRouter.onScreenShow` and renders the first not-yet-passed step whose `screen` matches the one that appeared, after a ~300ms settle (show-observers fire *before* the screen is un-hidden + slid in). `advanceClick` steps attach a delegated **capture-phase** click listener (fires before any `stopPropagation`; the advance is deferred a frame so the triggering click resolves first). A step may carry a `resolve(state)` callback (the director is given a `getState` getter): it runs at render time to rewrite the step's copy/target from live game state, or return `{ skip: true }` to drop the step (the director advances past it). Navigation stays in `main.ts`: the director only triggers the first jump (Home → team select) via the `onStartTour` callback passed to `initOnboarding()`; every later step is reached by the player acting on the real UI.

**State is global (localStorage), not save-scoped** — `rugbyOnboardingDone` (`'1'` once skipped/finished) + `rugbyOnboardingStep` (resume pointer). Onboarding is a property of the device/player; the two new-game entry points (Quick Start vs Career) produce separate saves, so a save-scoped flag would re-trigger the tour on every new game. **No `SAVE_VERSION` bump.** Replay via **Settings → Help → Replay guided tour** (`restartOnboarding()` clears the flag and jumps into team select). Wired once in `main.ts` (`initOnboarding`) before the first Home render so it catches the boot screen-show.

**Flow it walks.** *Phase 1 (get into the first match fast):* Home intro → team-selector → team-info → mode-picker (Quick Start) → **Hub: next-match card first** (know your opponent) → **the “?” help button** (where to find per-screen guidance if stuck) → **Squad** (squad-management walkthrough: select a player **by their number square** → swap → expand a card for quick attributes → open a full player profile → save) → Hub (Continue) → pre-match → **live match** (switch into the 2D pitch view for kickoff → the other views → the sim controls; non-blocking, never covers the pitch) → match-result. Tactics are deliberately **not** in the pre-match leg — they also surface in the build-up, so the tour skips straight to kickoff. *Phase 2 (season management), continuing straight on from the first result:* Hub → **Tactics** (the advanced/optional game-plan screens, now that they've played) → **Club → Assistant Manager** (how to delegate League Cup ties) → **Club → Staff** (a `resolve`-driven sequence: point out the staff salary budget and explain it's separate from the player wage budget; then, only if there's spare budget *and* a genuinely better affordable assistant in the pool, walk the player through releasing their current assistant and hiring the recommended one — otherwise it skips straight to a "backroom sorted" close, so it never gets stuck on an already-filled slot) → **Contracts & Transfers → Transfers** (explains the free-agent market; notes that Quick Start ships a complete squad with the wage budget committed, so signings come in future seasons or in Squad Builder mode) → **Contracts & Transfers → Loans** (send a youngster out on loan to develop) → finish. The whole thing is one continuous, resumable tour (the `rugbyOnboardingStep` pointer spans both phases); the final step is the Loans card. A step may set `returnToHub` so its button also navigates home (`onGoHub` callback) — used after the deep Staff step to spare a two-level menu back-out.

**New-user default.** While the tour is active (`isOnboardingActive()` — exported from the director), `onQuickStart` calls `gameEngine.setCupManageLive(true)` so the manager stays in charge of their own League Cup ties; otherwise the assistant sims them by default, which confused new managers (the board reacts to assistant-run results mid-tutorial).
