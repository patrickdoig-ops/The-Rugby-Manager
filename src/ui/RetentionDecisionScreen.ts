...
export function initRetentionDecisionScreen(
  ...
  function render(): void {
    ...
    // Plain Retain — no wage modal here. A retention only fires against
    // an external poach bid, but the off-season window never seeds
    // offers for the user's own players (openSigningWindow excludes the
    // player's club), so AI never poaches them and this screen is in
    // practice unreachable; even if reached, the player has no market
    // offer so the resolver's wage term is zero and a negotiated wage
    // would have no effect. Submitting at the default retention terms
    // keeps the path correct without a misleading negotiation UI.
