...
| **BoxKick** | `kickScore = kicking + rng(1,20)` ≥ 75 → very_good (contested, 20m); else poor (uncontested, 50-50 30m or 8m; catchScore + fullbackMod ≥ 35). `fullbackMod`: `three_back` +15, `two_back` +8, `one_back` 0 | contested: margin ≥ 10 attack_retain; ≥ 0 defend_knock_on; else defend_catch_contested. Uncontested: catchScore ≥ 35 defend_catch; else knock_on |
