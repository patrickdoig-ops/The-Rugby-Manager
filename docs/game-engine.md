...
| `PREM_CUP_ROUND_SHOWN` | After the player dismisses a cup round (leg / KO stage) summary in the weekly flow | Appends the round key (`leg:0`-`leg:2` \| `sf` \| `final`) to `premCup.shownRounds` so it doesn't re-surface. Mirrors `EUROPEAN_ROUND_SHOWN`. |
