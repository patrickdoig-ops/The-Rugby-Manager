...
**Team data.** `src/data/team-*.json` — currently Gloucester, Bristol, Leicester, Saracens; 15 starters + 8 bench each, 12 base stats on a 1–100 scale. `MatchCoordinator.initPlayer` copies `baseStats` to `currentStats` at match start; `StaminaSystem.computeFatigue` drives `currentStats` over the match via `FATIGUE_APPLIED`. `baseStats` is never modified. Player names are unique across both squads — `CommentaryFeed`'s colourisation pass relies on this.
