...
- *Overlay engine* (`PlayOverlay.ts`) — **⬜ outstanding.** On trigger (FirstPhase strike ball; clean quick ruck ball in PhasePlay) bind roles → agents, install the run-lines as their Layer-1 source for the play lifetime, schedule the actions through the WP5 pass mechanics; Layers 2–3 stay live; per-tick aborts revert to ShapeSolver targets. Outcome-affecting → golden + band re-baseline. Leans Opus.
