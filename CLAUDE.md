...
- **`CardHandler` runs before `PenaltyHandler` on every penalty.** Owned by `MatchCoordinator` like the other orchestrators. `evaluateNewPenalty()` rolls TMO (60% for high tackles) or team-22-rule (4th defensive penalty in own 22 → forced yellow); on a TMO trigger it transitions phase to `TmoReview` and the next 3 ticks drive narrative (`advanceTmoReview()`) with the clock frozen via `ClockController.advanceMinute` short-circuit. `scanSinBinReturns()` runs every non-TMO tick and fires `SIN_BIN_RETURNED` / `RED_20_EXPIRED`; red_20 expiry triggers `MatchCoordinator.handleRed20Replacement` (forced sub modal for human side, position-group auto-pick for AI / silent). All mutations flow through `CARD_ISSUED` / `SIN_BIN_RETURNED` / `RED_20_EXPIRED` / `TEAM_PENALTY_22_RECORDED` / `TEAM_22_WARNING_ISSUED` / `TMO_REVIEW_*` `MatchEvent` variants. Resolvers and selectors filter team rosters through `onFieldPlayers(team, state, side)` (`src/engine/FieldPosition.ts`) so a sin-binned forward genuinely weakens the scrum / lineout / breakdown (no separate flag on `Player`). Full breakdown in `docs/match-engine.md` § "Cards (Yellow / Red 20 / Red full)".
...
**Navigation flow.** Home → Team Selector → Hub → PreMatch → Match → Result → Round Results → League Table → Hub. The Hub (`src/ui/HubScreen.ts`) is the in-season control centre: six tiles route to Fixtures (`fixture-list`), League (`league-table`), Contracts (`contracts`, live as of v2.36a — sortable squad list with interactive marquee toggle + live cap pill (ok / tight / over)), and three remaining placeholder destinations (Squad / Training / Transfers — no-op handlers until those screens exist), plus a Settings cog and a "Go to next match" CTA. The post-match chain walks the manager through Round Results (`RoundResultsScreen` — every fixture of the just-completed round, player's match highlighted) then League Table (in a forward "Continue → Hub" mode set via `showLeagueTablePostMatch(onContinue)`) before landing on Hub. **When the final round of a season finishes, the chain detours**: LeagueTable → EndOfSeasonScreen (final standings + top scorer + MVP) → **`openRenewalWindow()` — if expiring contracts exist** → RenewalsScreen (per-row Renew/Release toggle on the player's expiring squad with live projected-cap pill) → `closeRenewalWindow(decisions)` (user decisions for the player's club + AI auto-resolve for everyone else via `AITransferDirector` — all in one event-batched call) → RolloverScreen (retirements + per-player aging deltas) → Hub for the new season. If no contracts expire, the Renewals detour is skipped and EndOfSeason flows straight into rollover. Detection is via the `game:seasonComplete` flag latched in `main.ts`; `rollSeason()` runs on the post-renewal transition and returns the applied `SeasonEvent[]` for the rollover screen to render. PreMatch back and the in-season sub-screen back-buttons all return to Hub. Settings has two entry points with different back targets — `goSettingsFromHome` (back → Home) and `goSettingsFromHub` (back → Hub) in `main.ts`.
...
**Contracts + cap (Phase 2 + Phase 3, live as of v2.36a).** Every roster Player carries `PlayerContract { clubId, expiresOn, annualWage, isMarquee }` + `reputation: number`, seeded once via `src/game/contractSeeder.ts::seedContractFields` from `rngTransfer` (wage = `WAGE_BY_RATING` × `POSITION_SCARCITY` × `WAGE_NOISE`, length age-banded, expiry staggered ~22/38/42 across +1/+2/+3 years). One hand-authored marquee per club via a `Marquee: yes.` annotation in `docs/team-data.md`, translated by `scripts/generateTeamJsons.mjs` into a `contract: { isMarquee: true }` JSON override that `contractSeeder` honours. `ContractsScreen` (Hub → Contracts) is interactive: tap-to-toggle marquee per row (goes through `MARQUEE_DESIGNATED` so the seam stays clean), 3-state cap pill (ok / tight at ≥ 95% / over). Cap usage = Σ(non-marquee wages); the marquee slot is excluded. Tuning constants in `src/engine/balance/transfers.ts`.

**End-of-season renewals (Phase 4, live as of v2.36a).** Between the final-round `EndOfSeasonScreen` and rollover, `gameEngine.openRenewalWindow()` emits `MARKET_OPENED` with one `TransferOffer` per expiring player league-wide (wage = fresh-market rate × `1 - loyaltyDiscount`, length re-derived from current age via `contractSeeder`). `RenewalsScreen` surfaces the player's club's offers with a per-row Renew/Release toggle (defaults to Renew) + live projected cap. On Continue, `closeRenewalWindow(decisions)` resolves every pending offer in one event-batched call: user decisions for the player's club, AI auto-resolve via `src/game/aiTransferDirector.ts::decideAIOffers` (pure, RNG-free greedy by OVR with marquee + cap-target + `aiReleaseRatingFloor` rules) for the rest. Accepted offers fire `OFFER_RESPONDED(accept)` + `CONTRACT_EXTENDED`; rejected fire `OFFER_RESPONDED(reject)` + `CONTRACT_TERMINATED('expired')` (player joins `state.career.freeAgents`, reserved for Phase 5 consumption). `MARKET_CLOSED` clears `state.career.market` and `rollSeason()` runs immediately after. Determinism: `rngTransfer` is consumed by `contractSeeder` during offer generation; decision logic itself is deterministic from the inputs. Tuning: `RENEWAL.loyaltyDiscount` / `aiTargetCapUtilisation` / `aiReleaseRatingFloor` in `balance/transfers.ts`.

**Save schema.** `SAVE_VERSION = 7` (as of v2.36a). v6 saves auto-migrate by defaulting `freeAgents` / `market` to `[]` / `null`; v5 saves additionally run `contractSeeder` over the persisted roster; v4 triggers a fresh roster seed; v3/v2 schedule-only restore; v1 discarded. The persisted career state always flows through `CAREER_ARCHIVE_RESTORED` (which accepts optional `freeAgents` + `market`), so the mutation seam holds. Full version table in `docs/game-engine.md` § "Save format".
...
### Transfer system (Phases 5-7)
- **Free-agent signings (Phase 5), cross-Prem poaching under Reg 7 (Phase 6), and generated player supply — academy graduates + foreign imports (Phase 7)** are sketched in **`docs/transfer-system.md`** with concrete file-by-file work items per phase. Phases 1-4 are live (multi-season rollover + read-only contracts + interactive marquee + end-of-season renewals); the seams left for Phases 5-7 are the same `applySeasonEvent` mutation boundary, the `rngTransfer` stream, and the existing `TransferOffer` shape — no new architectural primitives needed. The `state.career.freeAgents` pool already accumulates from Phase 4's unrenewed contracts, waiting for Phase 5's signing flow to consume it.
